...
function otExists($con, $date, $idacct){ 
	...
	$Qry->table ="tbltimeovertime";
	$Qry->selected ="id";
	$Qry->fields ="idacct='".$idacct."' AND docnumber NOT IN('ETOFILINGHOLIDAY','ETOFILINGRESTDAY') AND stat NOT IN (4,2) AND date='".$date."' AND id NOT IN (SELECT idapp FROM tblappcancel WHERE `type` = 'overtime' AND STATUS = 1) ";
	$rs = $Qry->exe_SELECT($con);
	return mysqli_num_rows($rs) >= 1;
}
