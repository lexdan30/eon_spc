...
WHEN a.hdate BETWEEN '".$date."' AND '".$date1."' THEN IF(whrs>0,IF((whrs - SUM(late+ut+`abs`))<0,0,(whrs - SUM(late+ut+`abs`))),0) 
...
function gettotalwhrs($con,$search,$date,$date1){
	...
	$Qry->fields =  "a.sc_stat='1'  AND a.id NOT IN(83,105,915) AND IF(whrs>0,((CASE
                                                                                    WHEN a.hdate BETWEEN '".$date."' AND '".$date1."' THEN whrs
                                                                                        ELSE 104
                                                                                    END) - ((late)+(ut)+(`abs`))),0) > 0 ". $search;
	$rs = $Qry->exe_SELECT($con);
	if(mysqli_num_rows($rs)>= 1){
