...
if($param['accountid'] != $param['info']['acct']){
    ...
    if($approver1 == $param['info']['acct']){
        $approver1 = getSuperiorid( $con,$param['info']['acct']);
    }
...
//print_r($approver2 . 'hi');


$Qry2 = new Query();	
$Qry2->table ="tblbunits";	
$Qry2->selected ="idhead";
$Qry2->fields ="name='Human Resources'";
                    ...
                    if( $param['info']['dateto'] ){
                        ...
                        if( $date1 <= $date2 ){
                            ...
                            foreach( $param['info']['leave_dates']  as $keys=>$value ){
                                ...
                                // $autoapp_approver1 = false;
                                // if($value['latefiled'] == 'true' &&  $idleave != 16){ //except birthdays
                                //     $approver1 = $getHR; //HR Manager
                                //     if($param['accountid'] != $param['info']['acct']){
                                //         $approver2 = $param['accountid'];//getApproverss($con, $param['info']['acct'], 'approver2', $param['info']['acct']);
                                //     }else{
                                //         $approver2 = getApproverss($con, $param['info']['acct'], 'approver1', $param['accountid']);
                                //     }
                                // }else{
                                    // $approver1 = getApproverss($con, $param['info']['acct'], 'approver1', $param['accountid']);
                                    // $approver2 = '';
                                    // if($param['accountid'] != $param['info']['acct']){
                                    //     $autoapp_approver1 = true;
                                    // }


                                //}

                                // if( DateTime::createFromFormat("Y-m-d", $date_create)->format("Y") != DateTime::createFromFormat("Y-m-d", $date)->format("Y")){
                                //     $reject[]	= array(
                                //         "date"	=> $date,
                                //         "msg"	=> "Cannot proceed previous/future year application."
                                //     );
                                // }
                                ...
                                $entitled_year = checkYearEntitlement($con, $idacct, $idleave);
                            ...
                            2023            2022 wrong
                                */
                                ...
                                $targdate = DateTime::createFromFormat("Y-m-d", $date_create)->format("Y-m-d") > '2023-04-30' ? '2018-07-01' : '2022-01-01';

                                if( $entitled_year == '2023' && DateTime::createFromFormat("Y-m-d", $date)->format("Y") == '2022'){
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Leave Balances {$entitled_year} is for the Year {$entitled_year} only."
                                    );
                                } else if($idleave != 17 && ($entitled_year == '2022' && DateTime::createFromFormat("Y-m-d", $date)->format("Y-m-d") >= "2023-05-01")){ //&& '2022' == DateTime::createFromFormat("Y-m-d", $date)->format("Y"))){
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Year ".$entitled_year." entitlement is not applicable for date ".$date."."
                                    );
                                } else if ($idleave == 1 && $value['latefiled'] == 'false' && getHireDate($con, $idacct) >= $targdate && DateTime::createFromFormat("Y-m-d", $date_create)->format("Y-m-d") < DateTime::createFromFormat("Y-m-d", $date)->format("Y-m-d")) {
                                    // 2018-07-01
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Cannot file SL in advance, " . DateTime::createFromFormat("Y-m-d", getHireDate($con, $idacct))->format("M d, Y") . " hire date is after July 2018."
                                    );
                                } else if( empty($approver1) ){
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Please setup classification in 201."
                                    );
                                }elseif( $value['shift'] == 'Rest Day' ){
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Cannot file leave on Rest Day."
                                    );
                                }elseif($value['allowbirthdayfile'] == 'false' &&  $idleave == 16){ //If Birthday leave is allowed
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Filing should be 30 days prior to the birth date or 30 days after the date of the birth date."
                                    );
                                } else if ($idleave == 9 && $date > DateTime::createFromFormat("Y-m-d", $date_create)->format('Y-m-d')) {
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Advance filing is not allowed"
                                    );
                                } elseif($idleave != 17 && ($entitled_year == '2022' && DateTime::createFromFormat("Y-m-d", $date)->format("Y-m-d") <= "2023-01-31")){ //&& '2022' == DateTime::createFromFormat("Y-m-d", $date)->format("Y"))){
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Year ".$entitled_year." entitlement is not applicable for date ".$date."."
                                    );
                                } elseif($idleave != 17 && ($entitled_year == '2023' && DateTime::createFromFormat("Y-m-d", $date)->format("Y-m-d") <= "2023-01-31")){ //&& '2022' == DateTime::createFromFormat("Y-m-d", $date)->format("Y"))){
                                    $reject[]	= array(
                                        "date"	=> $date,
                                        "msg"	=> "Year ".$entitled_year." entitlement is not applicable for date ".$date."."
                                    );
                                ...
                                elseif( (int)$value['hrs'] > 0 ){
                                    ...
                                    $hrs			= $value['hrs'];//(int)$value['hrs'];
                                    if( $hrs <= $leave_balance ){
                                        $total_units 	= $total_units + $value['hrs'];//(int)$value['hrs'];
                                        ...
                                        if( $autoapp_approver1 ){
                                            $Qry->selected 	= $Qry->selected . ", approver1_date";
                                            $Qry->fields 	= $Qry->fields 	 . ", '".$date_create."'";

                                            $Qry->selected 	= $Qry->selected . ", approver1_time";
                                            $Qry->fields 	= $Qry->fields 	 . ", '".SysTime()."'";

                                            $Qry->selected 	= $Qry->selected . ", approver1_stat";
                                            $Qry->fields 	= $Qry->fields 	 . ", '1'";

                                            $Qry->selected 	= $Qry->selected . ", stat";
                                            $Qry->fields 	= $Qry->fields 	 . ", '1'";

                                            $Qry->selected 	= $Qry->selected . ", date_approve";
                                            $Qry->fields 	= $Qry->fields 	 . ", '".$date_create."'";
                                        }

                                        // if($param['accountid'] != $param['info']['acct']){
                                        //     if($param['accountid'] == $approver1){
                                        //         $Qry->selected 	= $Qry->selected . ", approver1_date";
                                        //         $Qry->fields 	= $Qry->fields 	 . ",  '".SysDatePadLeft()."'";
...
function leaveExists($con, $date, $idacct, $idtimeleavetype){ // unused function
	$Qry = new Query();	
	$Qry->table ="tbltimeleaves";
	$Qry->selected ="*";
...
function hasAdjustment($con, $date, $idacct, $idtimeleavetype){ // unused function
	$Qry = new Query();	
...
function getPendingBalance($con, $idleave , $idacct){
	...
	$Qry->fields    = "idacct='".$idacct."' AND idleave='".$idleave."'";
	$rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getPendingBalance');
	if(mysqli_num_rows($rs)>= 1){
        return mysqli_fetch_assoc($rs)['pending_bal'];
	}
	return 0;
}
...
function getLastID($con, $ticket ){
	...
	$rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getLastID');
	if(mysqli_num_rows($rs)>=1){
...
function checkYearEntitlement($con,$idacct,$idleave){	

    $Qry = new Query();
    $Qry->table = "vw_leavesummary";
    $Qry->selected = "entitled_year";
    $Qry->fields = "idacct = '".$idacct."' AND idleave = '".$idleave."'";
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
        if($row=mysqli_fetch_assoc($rs)){
            return $row['entitled_year'];
        }
    }
    return '';
}
...
//     $rs = $Qry->exe_SELECT($con);
//     Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getApproverss');
//     if(mysqli_num_rows($rs)>= 1){
//         if($row=mysqli_fetch_assoc($rs)){
//            $idunit =  $row['idunit'];
...
//             $Qry2->selected ="approver_1a, approver_1b, approver_1c,
...
//             $rs2 = $Qry2->exe_SELECT($con);
//             Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs2), 'getApproverss(Qry2)');
//             if(mysqli_num_rows($rs2)>= 1){
//                 if($row2=mysqli_fetch_assoc($rs2)){
...
function geidhead($con,$id){
	...
	$Qry->fields ="id='".$id."'";
    $rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'geidhead');
    if(mysqli_num_rows($rs)>= 1){
...
function getHrid($con, $idleave){
    ...
    $rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getHrid');
    if(mysqli_num_rows($rs)>= 1){
        if($row=mysqli_fetch_assoc($rs)){
            $name =  $row['name'];
            ...
            $rs2 = $Qry2->exe_SELECT($con);
            Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs2), 'getHrid(Qry2)');
            if(mysqli_num_rows($rs2)>= 1){
                if($row2=mysqli_fetch_assoc($rs2)){
                    $approver = $row2['value'];
                    ...
                    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs3), 'getHrid(Qry3)');
                    if(mysqli_num_rows($rs3)>= 1){
                        return mysqli_fetch_assoc($rs3)['id'];
                    }
...
function getTimesheetPayPeriods( $con, $date ){
    ...
    $rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTimesheetPayPeriods');
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            if(!empty($row['id_payperiod'])){
                $data = array( 
                    "id"        => $row['id_payperiod'],
                );
            }else{
