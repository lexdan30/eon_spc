...
$pie_data	= array(getCS($con, $search, $param->accountid,$ids),getAA($con, $search, $param->accountid,$ids),getOT($con, $search, $param->accountid,$ids),getLV($con, $search, $param->accountid,$ids),getOB($con,$search, $param->accountid,$ids));
...
function getOT($con,$pay_period, $idacct, $ids){
    $pay_period = getPayPeriod($con);
    $newidunits = getClassHeadsids($con, $idacct);
    $Qry=new Query();
    $Qry->table="vw_overtime_approval";
    $Qry->selected="COUNT(id) as ctr";
    $Qry->fields="id>0 AND (date BETWEEN '".$pay_period['pay_start']."' AND '".$pay_period['pay_end']."') AND idacct !='".$idacct."' AND stat=3 AND (idunit IN (".$newidunits.") OR (idsuperior = '" .$idacct."'))";
    //$Qry->fields="id>0 AND idacct !='".$idacct."' AND stat=3";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getOT');
    if(mysqli_num_rows($rs)>=1){
...
function getCS($con,$pay_period, $idacct,$ids){
    $pay_period = getPayPeriod($con);
    $newidunits = getClassHeadsids($con, $idacct);
    $Qry=new Query();
    $Qry->table="vw_shift_application";
    $Qry->selected="COUNT(id) as ctr";
    $Qry->fields="id>0 AND (date BETWEEN '".$pay_period['pay_start']."' AND '".$pay_period['pay_end']."') AND idacct !='".$idacct."' AND stat=3 AND (idunit IN (".$newidunits.") OR (approver1 = '" .$idacct."'))";
    //$Qry->fields="id>0 AND idacct !='".$idacct."' AND stat=3";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getCS');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return 0;
...
function getAA($con,$pay_period, $idacct,$ids){
    $pay_period = getPayPeriod($con);
    $newidunits = getClassHeadsids($con, $idacct);
    $Qry=new Query();
    ...
    //$Qry->fields="id>0 AND idacct !='".$idacct."' AND stat=3";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getAA');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return 0;
...
function getOB($con,$pay_period, $idacct,$ids){
    $pay_period = getPayPeriod($con);
    $Qry=new Query();
    $Qry->table="vw_ob_application";
    $Qry->selected="COUNT(id) as ctr";
    $Qry->fields="((approver1_id = '" .$idacct."') OR 
                  (approver2_id = '" .$idacct."')) AND (date BETWEEN '".$pay_period['pay_start']."' AND '".$pay_period['pay_end']."') AND idacct!='".$idacct."'AND stat=3";// AND idunit IN (".$ids.")
    //  $Qry->fields="((approver1_id = '" .$idacct."') OR 
    //                (approver2_id = '" .$idacct."')) AND idacct!='".$idacct."'AND stat=3 ";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getOB');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return 0;
...
function getLV($con, $pay_period, $idacct,$ids){
    $pay_period = getPayPeriod($con);
    ...
    $Qry->fields="((  find_in_set('".$idacct."',approver1) AND idacct != '".$idacct."' AND idacct != '".$accountidd."') OR 
                    ...
                    ) AND (date BETWEEN '".$pay_period['pay_start']."' AND '".$pay_period['pay_end']."') AND stat=3 "; //AND idunit IN (".$ids.")
    ...
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getLV');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return 0;
...
function getClassHeadsids($con, $idaccts){
    $id = array();
    $Qry=new Query();
    $Qry->table="tblbunits";
    $Qry->selected="id";
    $Qry->fields="idhead = '".$idaccts."'";
    $rs=$Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>=1){
