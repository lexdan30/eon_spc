					...
					if( $param->info->start_time ){
						if( $param->info->end_time ){
							...
							if( $date1 <= $date2 ){
								...
								if( otExists($con, $param->info->otsdate, $param->info->emp) ){
									$reject[]	= array(
								...
								}else if( strtotime( getschedinfo($con,$param->info->emp, $param->info->otsdate)['out'] ) == strtotime( $param->info->start_time ) || (int)$shift_info[0] == 4 || getschedinfo($con,$param->info->emp, $param->info->otsdate)['driver'] == true || getschedinfo($con,$param->info->emp, $param->info->otsdate)['fallonweekdayholiday']){								
									$docnumber		= "OT". $param->accountid .strtotime( $date_create.$time ).$time;	
									$idacct			= $param->info->emp;
									...
									if(getschedinfo($con,$param->info->emp, $param->info->otsdate)['fallonweekdayholiday'] && getschedinfo($con,$param->info->emp, $param->info->otsdate)['site'] != '1' && (strtotime( $sdate.' '.$stime.'' ) < strtotime( $sdate.' '.'07:00'.'' )) &&  getschedinfo($con,$param->info->emp, $param->info->otsdate)['driver'] == false){
										$stime = '07:00';
									} 

									if(getschedinfo($con,$param->info->emp, $param->info->otsdate)['flexi'] && getschedinfo($con,$param->info->emp, $param->info->otsdate)['site'] != '1' && getschedinfo($con,$param->info->emp, $param->info->otsdate)['joblevel'] == 'Mgr' && getschedinfo($con,$param->info->emp, $param->info->otsdate)['fallonweekdayholiday'] == false){ 
										if((strtotime( $fdate.' '.$ftime.'' ) > strtotime( $fdate.' '.'18:00'.'' ))){
											$planhrs		= sprintf('%0.2f', ((strtotime( $fdate.' '.$ftime.'' ) - strtotime( $sdate.' '.$stime.'' )) / ( 60 * 60 )) - 1 );
										}elseif((strtotime( $fdate.' '.$ftime.'' ) < strtotime( $fdate.' '.'18:00'.'' )) && (strtotime( $fdate.' '.$ftime.'' ) > strtotime( $fdate.' '.'17:00'.'' ))){
									...
									}elseif(getschedinfo($con,$param->info->emp, $param->info->otsdate)['fallonweekdayholiday'] && getschedinfo($con,$param->info->emp, $param->info->otsdate)['joblevel'] == 'RnF'){
										if((strtotime( $sdate.' '.$stime.'' ) < strtotime( $sdate.' '.'12:00'.'' )) && (strtotime( $fdate.' '.$ftime.'' ) > strtotime( $fdate.' '.'13:00'.'' ))){
											$planhrs		= sprintf('%0.2f', ((strtotime( $fdate.' '.$ftime.'' ) - strtotime( $sdate.' '.$stime.'' )) / ( 60 * 60 )) - 1 );
										}elseif((strtotime( $sdate.' '.$stime.'' ) < strtotime( $sdate.' '.'12:00'.'' )) && ((strtotime( $fdate.' '.$ftime.'' ) < strtotime( $fdate.' '.'13:00'.'' )) && (strtotime( $fdate.' '.$ftime.'' ) > strtotime( $fdate.' '.'12:00'.'' )))){
											$planhrs		= sprintf('%0.2f', ((strtotime( $fdate.' '.'12:00'.'' ) - strtotime( $sdate.' '.$stime.'' )) / ( 60 * 60 )) );
											$ftime = '12:00';
										}elseif((strtotime( $sdate.' '.$stime.'' ) < strtotime( $sdate.' '.'13:00'.'' )) && ((strtotime( $fdate.' '.$stime.'' ) > strtotime( $fdate.' '.'12:00'.'' )) && (strtotime( $fdate.' '.$ftime.'' ) > strtotime( $fdate.' '.'13:00'.'' )))){
									...
									//if( $planhrs >= 1 && $planhrs <= 16 ){
										...
										$Qry->selected 	= "creator, docnumber,  idacct, idshift,  date, sdate, fdate, stime, ftime, planhrs, date_create,approver1,approver1_date,approver1_time,approver1_stat,stat";
										...
										,'".$param->accountid."', '".SysDatePadLeft()."', '".SysTime()."', '1', '1'";
										if( !empty( $remarks ) ){
									...
									$return = json_encode(array('status'=>'invtime','msg'=>'Start Time should be ' . date("h:i:s a",strtotime(getschedinfo($con,$param->info->emp, $param->info->otsdate)['out'])), 'out'=>$shiftout,  'os'=>$param->info->start_time ));
								}
							}else{
...
AS loginschedref,
CAST( ADDTIME( ( CASE 
			WHEN a.timein IS NULL  OR a.timein = ''  THEN `fpshift`.`ftime` 
