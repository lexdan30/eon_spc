...
$Qry->selected  = "pr.*,CONCAT(`a`.`lname`,IFNULL(CONCAT(' ',`a`.`suffix`),''),', ',`a`.`fname`,' ',SUBSTR(`a`.`mname`,1,1),'. ') AS `empname`, a.empid as empid, 
                   ...
                   ";
$Qry->fields    = "pr.idpayperiod = '" . $id_paydate  . "' AND pr.type = '" . $type  . "' AND pr.idpaygrp = '" . $paygroup  . "'" .$search . "  ORDER BY empname LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize ."";
...
function getTotal($con,$idpayperiod,$param){
    ...
    $Qry->table         = "tblpayreg as pr LEFT JOIN tblaccount as a ON pr.idacct = a.id LEFT JOIN tblaccountjob as aj ON aj.idacct = a.id ";
    $Qry->selected      = "*";
    $Qry->fields        = "idpayperiod = '" . $id_paydate . "' AND type = '" . $type . "' AND pr.idpaygrp = '" . $paygroup . "'" . $_SESSION['search'];
    $rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
...
function getGrandtotal($con,$idpayperiod,$param){
	...
	if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            $leaves = array('tc_sl','tc_vl','tc_al','tc_svl','tc_lwop','tc_mlwop','tc_spl','tc_pl','tc_slh','tc_ul','tc_il','tc_el','tc_mc','tc_bl','tc_ml','tc_s','tc_psl','tc_vvawc','tc_bdl','tc_cl','tc_obt');
