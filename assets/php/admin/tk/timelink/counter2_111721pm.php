...
$data = array( 
 ...
 // print_r($data['period']['pay_start']);
// $return = $data['period']['pay_start'].' '. $data['period']['pay_end'];
// print $return;
// mysqli_close($con);
...
if( !empty($param->accountid) ){
	...
	if( !empty($arr) ){
		...
		$Qry->fields    = "a.idunit > 0 AND a.work_date BETWEEN '".$data['period']['pay_start']."' AND '".$data['period']['pay_end']."'";
		if( empty($unit) && empty($empname) && empty($batchno)){
		...
		if( !empty($batchno) ){
			//$nameemp = $param->info->empname;
			$Qry->fields    = $Qry->fields    . " AND a.batchid = '".$batchno."'";
		}
		...
		if(mysqli_num_rows($rs)>=1){
			while($row=mysqli_fetch_array($rs)){
				$data[] = array(
					...
					"range"		=> getDateRange($con,$data['period']['pay_start'],$data['period']['pay_end'])
				);
			}
		}
	}
