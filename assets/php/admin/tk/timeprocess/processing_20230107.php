...
//  WHRS WHEN lateref + utref != 0 THEN IF( (acthrs - excess) + (lateref + utref)  > shifthrs, shifthrs,(acthrs - excess) + (lateref + utref) )
...
// SUM((CASE 
// WHEN leaveappstatus = 1 AND  idtimeleavetype = 1 THEN `leave`
// WHEN timein IS NOT NULL AND timeout IS NOT NULL AND leaveappstatus = 1 AND `leave` = 4 AND acthrs > (shifthrs / 2) THEN ((shifthrs / 2) - (late + ut))
// WHEN holidaytype IS NOT NULL THEN IF( (acthrs - excess)  > shifthrs, shifthrs,(acthrs - excess) )
// WHEN lateref + utref > 1 THEN  IF( (acthrs - excess) > shifthrs, shifthrs, (acthrs - excess))  
// WHEN leaveappstatus = 1 AND `leave` = 4 AND acthrs > (shifthrs / 2) THEN ((shifthrs / 2) - (late + ut))  
// WHEN lateref + utref < 1 AND leaveappstatus = 1 AND idtimeleavetype <> 1 THEN

// IF(IF(lateref + utref <= 1,IF(lateref + utref != 0,acthrs+lateref + utref,acthrs),acthrs) - (excess +lateref + utref) + IF(shifttype = 'Compressed Schedule',`leave`,4) >shifthrs,shifthrs
// ,IF(lateref + utref <= 1,IF(lateref + utref != 0,acthrs+lateref + utref,acthrs),acthrs) - (excess +lateref + utref) + IF(shifttype = 'Compressed Schedule',`leave`,4))

// WHEN lateref + utref < 1  THEN IF(IF(lateref + utref <= 1,IF(lateref + utref != 0,acthrs+lateref + utref,acthrs),acthrs) - (excess +lateref + utref) > shifthrs, shifthrs,IF(lateref + utref <= 1,IF(lateref + utref != 0,acthrs+lateref + utref,acthrs),acthrs) - (excess +lateref + utref))
...
$Qry->selected  = "tid,
                        ...
                        SUM((CASE 
                                ...
                                ELSE ut
                            END)) AS Undertime,
                        SUM(IF(leaveappstatus = 1 AND leaveidtype NOT IN(3,32) AND `leave` = 4 AND acthrs = 0.000000 AND idshift != 66,IF( absent - `leave` <= 0 ,0,absent - `leave`),IF(leaveidtype IN(3,32),`leave`,IF(absent >= 8 ,8,absent)))) AS `Abs`,
                        SUM(IF(leaveappstatus = 1 AND leaveidtype NOT IN(3,32) AND `leave` = 4 AND acthrs = 0.000000 AND idshift != 66,IF( absent - `leave` <= 0 ,0,absent - `leave`),IF(leaveidtype IN(3,32),`leave`,IF(absent >= 8,1,IF(absent >= 4,.5,0))))) AS `Absref`,
                        SUM((CASE 
                                WHEN holidaytype IS NOT NULL THEN IF( (acthrs - excess)  > shifthrs, shifthrs,(acthrs - excess) )
                                WHEN lateref + utref > 1 THEN  IF( (acthrs - excess) > shifthrs, shifthrs, (acthrs - excess))  
                                WHEN lateref + utref < 1  THEN IF(IF(lateref + utref <= 1,IF(lateref + utref != 0,acthrs+lateref + utref,acthrs),acthrs) - (excess +lateref + utref) > shifthrs, shifthrs,IF(lateref + utref <= 1,IF(lateref + utref != 0,acthrs+lateref + utref,acthrs),acthrs) - (excess +lateref + utref))
                        ...
                        SUM(IF(leaveidtype = 35 AND leaveappstatus = 1  AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ), `leave`, 0)) AS TC_MH,
                        SUM(IF(leaveidtype = 36 AND leaveappstatus = 1  AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ), `leave`, 0)) AS TC_SVL,
                        ...
                        WHEN ((idshift != 4) OR ((idshift IS NULL AND defaultschedid != 4))) AND holidaytype = 'SPECIAL' AND otstatus = 1 AND otapprove BETWEEN period_start AND period_endref THEN 
                                       IF((othrsp > shifthrs) ,shifthrs,othrsp)
                                    ELSE 0
                                   END))
                        AS H_SH,
                        SUM((CASE 
                        ...
                        SUM((CASE 
                            WHEN ((idshift != 4) OR ((idshift IS NULL and defaultschedid != 4))) AND holidaytype = 'SPECIAL' AND otstatus = 1 AND otapprove BETWEEN period_start AND period_endref THEN 
                            IF((othrsp - shifthrs) > 2 ,othrsp - shifthrs,0)
                            ELSE 0
                            END)) 
                        AS OT_SH,
                        SUM((CASE 
                        ...
                        AS OT_LSHRD,
                        SUM((CASE 
                            WHEN ((idshift != 4) OR ((idshift IS NULL and defaultschedid != 4))) AND holidaytype IS NULL THEN np
                            ELSE 0
                            END)) 
                        AS NP_NPReg,
                        SUM((CASE 
                        ...
                        AS NP_RD,
                        SUM((CASE 
                            WHEN ((idshift != 4) OR ((idshift IS NULL and defaultschedid != 4))) AND holidaytype = 'SPECIAL'  THEN np
                            ELSE 0
                            END)) 
                        AS NP_SH,
                        SUM((CASE 
                        ...
                        AS NP_SHRD,
                        SUM((CASE 
                            WHEN ((idshift != 4) OR ((idshift IS NULL and defaultschedid != 4))) AND holidaytype = 'LEGAL'  THEN np
                            ELSE 0
                            END)) 
                        AS NP_LH,
                        SUM((CASE 
                        ...
                        AS NP_LHRD,
                        SUM((CASE 
                            WHEN ((idshift != 4) OR ((idshift IS NULL and defaultschedid != 4))) AND holidaytype = 'LEGAL SPECIAL'  THEN np
                            ELSE 0
                            END)) 
                        AS NP_LSH,
                        SUM((CASE 
                        ...
                        SUM((CASE 
                            ...
                            END)) 
                        AS NPOT_LSHRD";
                        // AND tid IN(627,626,706,576,619)
...
if(mysqli_num_rows($rs)>= 1){
    ...
    while($row=mysqli_fetch_array($rs)){ 
        ...
        //$row['WHrs'] = $row['WHrs'] + $row['aaref'] + $row['TC_SL'] + $row['TC_AL'] + $row['TC_SPL'] + $row['TC_PL'] +$row['TC_SLH'] + $row['TC_UL'] + $row['TC_IL'] +  $row['TC_EL'] +  $row['TC_MC'] +  $row['TC_BL'] +  $row['TC_ML'] +  $row['TC_S'] + $row['TC_PSL'] + $row['TC_VVAWC']  + $row['TC_BDL']  + $row['TC_CL'];
        
        //$row['WHrs'] = $row['WHrs'] + $row['aaref'] + $row['TC_SL']  + $row['TC_VL'] + $row['TC_AL'] + $row['TC_SPL'] + $row['TC_PL'] +$row['TC_SLH'] + $row['TC_UL'] + $row['TC_IL'] +  $row['TC_EL'] +  $row['TC_MC'] +  $row['TC_BL'] +  $row['TC_ML'] +  $row['TC_S'] + $row['TC_PSL'] + $row['TC_VVAWC']  + $row['TC_BDL']  + $row['TC_CL'];
                ...
                if($row1=mysqli_fetch_array($rs1)){
                    ...
                    if($row['absent'] != 0){
                        ...
                        if($row1['obtriptype'] == 1){
                            ...
                            if($row1['leavename'] == 'Mental Health Leave '){
                                $val = $row1['leave'];
                                updateadjustmentscol($con,'adj_mh',$val,$empid,$lastid);
                            }
