...
//print_r($pay_period['pay_start'].' to '.$pay_period['pay_end']);
...
$Qry 			= new Query();	
$Qry->table     = "
(SELECT id, empid, sexstr, idunit, pic, lname, fname, mname, empname, post, etypeid, idsuperior FROM vw_dataemployees) AS de 
LEFT JOIN (SELECT tid AS empID, temp, leaveidtype AS idleave, holiday_type, leaveappstatus AS leavestat, work_date, fshfname AS shift_status,
...
$Qry->fields    = "de.etypeid=1 AND de.id != '".$param->accountid."'".$search." AND (de.idunit IN (".$ids.") or de.idsuperior='".$param->accountid."') AND
IF((SELECT 
GROUP_CONCAT(reso_txt) 
FROM
vw_resocenter 
