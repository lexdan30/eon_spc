...
$search='';
if( !empty( $param->search_acct  ) ){ $search.=" AND de.id =  '".$param->search_acct ."' "; }
if( !empty( $param->search_post  ) ){ $search.=" AND de.post =  '".$param->search_post ."' "; }
if( !empty( $param->search_dept  ) ){ $search.=" AND IF(bu.unittype = 3, bu.name, (SELECT `name` FROM tblbunits WHERE id = bu.idunder)) =  '".$param->search_dept ."' "; }
...
$Qry->fields    = "de.etypeid=1 AND dt.work_date = '".$date."' ". $search . "order by dept, sec, de.empname";
