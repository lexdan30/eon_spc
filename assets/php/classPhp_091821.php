...
function getAllAccounts($con, $id){
		...
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "*";
		$Qry->fields    = "etypeid IN(1,2,3) ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_array($rs)){
				$data[] = array( 
	...
	function getHeadPicName($con,$id,$unit){
		...
		if(mysqli_num_rows($rs)>= 1){
			...
			if($row=mysqli_fetch_array($rs)){
				...
				if($row['idemptype'] == 3){
					return  '<img src="assets/php/admin/hr/employee/pix/undefined.webp?1591229368" style="margin-right: 5px; width: 30px; height: 30px; border-radius: 50%;" class="user-image mar0"><label style="color:yellow;">Vacant Position</label><br>'.getPosition($con, $unit);
				}
				else{
					return  '<img src="assets/php/admin/hr/employee/pix/'.$row['pic'].'?1591229368" style="margin-right: 5px; width: 30px; height: 30px; border-radius: 50%;" class="user-image mar0">'.$row['lname'].", ". $row['fname']." ". $row['mname'].'<br>'.getPosition($con, $unit);
				}
			}
		}		
	...
	function getShiftIDData($con, $idshift){
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_array($rs)){
				$data = array($row['id'],$row['name'],$row['stime'],$row['ftime'],$row['break'],$row['reghrs'],$row['stype']);
			}
		}
		return $data;
	}
