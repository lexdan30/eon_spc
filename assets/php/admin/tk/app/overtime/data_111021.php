...
if( $param['order'][0]['column'] !='' ){//default 
	...
	$search=$search." ORDER BY ". $arrCols[$param['order'][1]['column']] ." ".$param['order'][1]['dir'];//default
}
...
if(mysqli_num_rows($rs)>= 1){
    ...
    while($row=mysqli_fetch_array($rs)){
        ...
        $data["data"][] = array(
			...
			'units'				=> getActualHours( $con,$row['idacct'],$row['date']),
...
function getschedinfo($con, $id, $date){ 
	...
	$Qry->selected  = "stime,ftime, idshift,breakin,breakout,site,idlvl,date_in,timein,date_out,timeout";
			...
			if($row['idshift'] != '89' && $row['idshift'] != '93'){
			 ...
			 if(!empty($row['timeout']) && (strtotime( $date.' '.substr($out,0,5).'' ) < strtotime($row['date_out'].' '.$row['timeout'].'' ))){
			  ...
			  $shiftout = array(				
				"in"	=> $row['stime'],
				"out"  => $startot,	
				"actdouthr"  => $row['date_out'],	
				"actouthr"  => $row['timeout'],				
				"br1"	=>  $row['breakin'],
				"br2"	=>  $row['breakout'],
				"site"=>  $site,
				"joblevel"=> $joblevel,
...
END ) AS breakoutref,  `e`.`site` AS `site`,  `e`.`idlvl` AS `idlvl`,`a`.timein, `a`.timeout,`a`.date_out
	";
