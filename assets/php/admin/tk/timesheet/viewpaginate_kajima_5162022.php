...
if( !empty( $param->filter->idlvl ) ){ $search=" AND idlvl =  '".$param->filter->idlvl."' "; }
$where = $search;
...
$Qry->selected  = "*";
$Qry->fields    = "id>0". $search . " ORDER BY empname LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize;
    ...
    if(mysqli_num_rows($rs)>=1){
        while($row=mysqli_fetch_assoc($rs)){
            ...
            $data[] = array(
                ...
                'getChangeShift'        => getChangeShift($row, $con ,$date ,$row['idpayperiod'] ,$row['work_date'],$row['tid'],$row['cs'],$row['csstatus'],$row['csname']),
                'getAttendanceAdjust'   => getAttendanceAdjust($row, $con ,$date ,$row['idpayperiod'] , $row['work_date'],$row['tid'], $row['shifttype'], $row['aastatus'], $row['aastime'], $row['aaftime'], $row['aasstime'], $row['aasftime'], $row['timein'], $row['timeout'], $row['timein2'], $row['timeout2']),
                'getOvertime'           => getOvertime($row, $con ,$date ,$row['idpayperiod'] , $row['work_date'],$row['tid'],$row['otstatus'], $row['othrs']),
                'getTimesheetLeaves'    => getTimesheetLeaves($row, $con ,$date ,$row['idpayperiod'] , $row['work_date'], $row['tid'], $row['leaveappstatus'], $row['leavename'], $row['leave']),
                'getObTrip'             => getObTrip($row, $con ,$date ,$row['idpayperiod'] , $row['work_date'],$row['tid'],$row['obtripstatus'],$row['obtrip'],$row['obstart'],$row['obend'])
            );
        }
    }else{
        return $data[] = array();
...
function getChangeShift($row, $con ,$date, $idpayperiod , $work_date, $idacct,$idshift, $status, $name){
    ...
    $data = array(         
        "status" 	=> $status,
        "name" 	    => $name,
        "late"      =>getlateapproveChangeShift($con ,$date , $work_date, $idacct, $idpayperiod)
    );

    return $data;
}

function getAttendanceAdjust($row, $con ,$date, $idpayperiod , $work_date, $idacct ,$shifttype ,$status, $stime, $ftime, $sstime, $sftime, $timein, $timeout, $timein2, $timeout2){
    $adaj = '';
        ...
        $data = array(
            ...
            "sstime" 	    => $sstime,
            "sftime" 	    => $sftime,
            "late"          =>getlateapproveAttendanceAdjust($con ,$date , $work_date, $idacct, $idpayperiod)
        );
...
function getOvertime($row, $con ,$date, $idpayperiod , $work_date, $idacct,$status,$hrs){
    if($row['otapprove']){
    ...
    $data = array(
        "status" 	    => $status,
        "planhrs" 	    => $hrs,
        "late"          =>getlateapproveOvertime($con ,$date , $work_date, $idacct, $idpayperiod)
    );

    return $data;
}
//OB
function getObTrip($row, $con ,$date, $idpayperiod , $work_date, $idacct,$status,$remarks,$start,$end){
    if($row['obapprove']){
    ...
    $data = array(
        ...
        "late"      =>getlateapproveObTrip($con ,$date , $work_date, $idacct, $idpayperiod)
    );
...
function getTimesheetLeaves($row, $con ,$date, $idpayperiod , $work_date, $idacct, $status, $name, $hrs){
    if($row['lvapprove']){
    ...
    $data = array(
                    ...
                    "name" 	    => $name,
                    "hrs" 	    => $hrs,
                    "late"        =>getlateapproveTimesheetLeaves($con ,$date , $work_date, $idacct, $idpayperiod)
                );
    return $data;
}
...
function getlateapproveAttendanceAdjust($con ,$date , $work_date, $idacct, $idpayperiod){
    $idpayperiod =  $idpayperiod - 1;
    $data = array();
    $Qry=new Query();
    ...
    and (approver1_date > (CASE 
                                ...
                                END)  
        OR app.id_payperiod < '".$idpayperiod."'
        )
...
function getlateapproveChangeShift($con ,$date , $work_date, $idacct, $idpayperiod){
    $idpayperiod = $idpayperiod - 1;
    $data = array();
    $Qry=new Query();
    ...
    and (approver1_date > (CASE 
                                ...
                                ELSE DATE_ADD('".$date."', INTERVAL IFNULL((SELECT `value` FROM `tblpreference` WHERE alias = 'NMAGP'),0) DAY)
                            END) 
          OR cs.id_payperiod < '".$idpayperiod."'
        )
...
function getlateapproveOvertime($con ,$date , $work_date, $idacct, $idpayperiod){
    $idpayperiod = $idpayperiod - 1;
    $data = array();
    $Qry=new Query();
...
function getlateapproveTimesheetLeaves($con ,$date , $work_date, $idacct, $idpayperiod){
    $idpayperiod = $idpayperiod - 1;
    $data = array();
    $Qry=new Query();
    ...
    and (date_approve > (CASE 
                            ...
                            ELSE DATE_ADD('".$date."', INTERVAL IFNULL((SELECT `value` FROM `tblpreference` WHERE alias = 'NMAGP'),0) DAY)
                        END) 
            OR lv.id_payperiod < '".$idpayperiod."'
        )
...
function getlateapproveObTrip($con ,$date , $work_date, $idacct, $idpayperiod){
    $idpayperiod = $idpayperiod - 1;
    $data = array();
    $Qry=new Query();
