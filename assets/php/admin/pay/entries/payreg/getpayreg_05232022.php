...
$Qry->selected  = "pr.*,CONCAT(`a`.`lname`,IFNULL(CONCAT(' ',`a`.`suffix`),''),', ',`a`.`fname`,' ',SUBSTR(`a`.`mname`,1,1),'. ') AS `empname`, a.empid as empid, 
                    ...
                    (mallowance) AS othertaxincome_hrs,
                    (mallowance_amount) AS othertaxincome_amount,
                    (adj_mc + adj_el + adj_cl + adj_ml + adj_pl + adj_spl + adj_vl + adj_bl + tc_vl + tc_spl + tc_sl  + tc_ml + tc_lwop + tc_el + tc_cl + tc_bl) AS otherNTincome_hrs,
                    (adj_mc_amount + adj_el_amount + adj_cl_amount + adj_ml_amount + adj_pl_amount + adj_spl_amount + adj_vl_amount + adj_bl_amount + tc_vl_amount + tc_spl_amount + tc_sl_amount + tc_ml_amount + tc_lwop_amount + tc_el_amount + tc_cl_amount + tc_bl_amount) AS otherNTincome_amount";
$Qry->fields    = "pr.idpayperiod = '" . $id_paydate  . "' AND pr.type = '" . $type  . "' " .$search . "  ORDER BY empname LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize ."";
...
if(mysqli_num_rows($rs)>= 1){
    ...
    $myData = array('status' => 'success', 
                    ...
                    'totalItems' => getTotal($con , $idpayperiod),
                    'grandtotal' => getGrandtotal($con ,$idpayperiod),
	...
	// $return = json_encode(array('status' => 'error', 'mysqli_error' => $param));
    $return = json_encode(array('status' => 'error', 'mysqli_error' => $Qry->fields ));
...
function getTotal($con,$idpayperiod){
    $id_paydate = $idpayperiod['period']['id'];;
    $type = $idpayperiod['period']['type'];;
    ...
    $Qry->table         = "tblpayreg as pr LEFT JOIN tblaccount as a ON pr.idacct = a.id LEFT JOIN tblaccountjob as aj ON aj.idacct = a.id ";
    $Qry->selected      = "*";
    $Qry->fields        = "idpayperiod = '" . $id_paydate . "' AND type = '" . $type . "'" . $_SESSION['search'];
    $rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
...
function getGrandtotal($con,$idpayperiod){
    $id_paydate = $idpayperiod['period']['id'];;
    $type = $idpayperiod['period']['type'];;
    ...
    $Qry->selected  = "SUM(pr.salary) as salary, 
                        ...
                        SUM(late) as late,
                            ...
                            SUM(adj_mc + adj_el + adj_cl + adj_ml + adj_pl + adj_spl + adj_vl + adj_bl + tc_vl + tc_spl + tc_sl  + tc_ml + tc_lwop + tc_el + tc_cl + tc_bl + tc_cvl + tc_menstl + tc_wl) AS otherNTincome_hrs,
                            SUM(adj_mc_amount + adj_el_amount + adj_cl_amount + adj_ml_amount + adj_pl_amount + adj_spl_amount + adj_vl_amount + adj_bl_amount + tc_vl_amount + tc_spl_amount + tc_sl_amount  + tc_ml_amount + tc_lwop_amount + tc_el_amount + tc_cl_amount + tc_bl_amount + tc_cvl_amount + tc_menstl_amount + tc_wl_amount) AS otherNTincome_amount,
                            SUM(ut) as ut,
                            SUM(abs) as abs,
                            SUM(whrs) as whrs,
                            SUM(tc_vl) as tc_vl,
                            ...
                            SUM(tc_cvl_amount) as tc_cvl_amount,
                            SUM(tc_menstl_amount) as tc_menstl_amount,
                            SUM(tc_wl_amount) as tc_wl_amount,
                            ...
                            SUM(adj_cvl_amount) as adj_cvl_amount,
                            SUM(adj_menstl_amount) as adj_menstl_amount,
                            SUM(adj_wl_amount) as adj_wl_amount,
	...
	if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            $leaves = array("tc_vl" ,"tc_sl" ,"tc_cl" ,"tc_ml" ,"tc_mc" ,"tc_pl" ,"tc_el" ,"tc_bl" ,"tc_obt" ,"tc_bdl" ,"tc_spl" ,"tc_lwop","tc_cvl","tc_menstl","tc_wl" );
