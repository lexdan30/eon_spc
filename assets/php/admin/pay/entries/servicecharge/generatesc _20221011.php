...
$Qry->selected  = "a.id,a.empid,a.empname,a.lname,a.fname,a.mname,IF(whrs>0,IF((104 - SUM(late+ut+`abs`))<0,0,(104 - SUM(late+ut+`abs`))),0)  AS totwhrs,(adj_absent - SUM(adj_late + adj_ut)) + (".$adj_leaves.") AS adj_totwhrs"; //IF(pa.transactiontype = 'UD',pa.amount,0.00) AS uniondues //+ (".$leaves.")
...
function getTotal($con,$search,$leaves){
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="a.empname,IF(whrs>0,(104 - SUM(late+ut+`abs`)),0) AS totwhrs"; // + (".$leaves.")
...
function getTotalEmpDetails($con,$search,$scrate,$leaves){
	...
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="a.empname,IF(whrs>0,IF((104 - SUM(late+ut+`abs`))<0,0,(104 - SUM(late+ut+`abs`))),0) AS totwhrs"; //+ (".$leaves.")
...
function gettotalwhrs($con,$search){
	...
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="(SUM(104) - (SUM(late)+SUM(ut)+SUM(`abs`))) AS totwhrs"; // + (".$leaves.")
	$Qry->fields =  "a.sc_stat='1'  AND a.id NOT IN(83,105,915) AND IF(whrs>0,(104 - ((late)+(ut)+(`abs`))),0) > 0 ". $search;
	$rs = $Qry->exe_SELECT($con);
	if(mysqli_num_rows($rs)>= 1){
