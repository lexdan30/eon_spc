...
SUM(IF(leaveidtype = 35 AND leaveappstatus = 1  AND ((lvapprove BETWEEN period_start AND period_endref)), `leave`, 0)) AS TC_MH,
...
if(!empty( $param->info->employee)){
    $Qry->fields    = "(separationdate > '".$date1."' OR separationdate IS NULL) AND idunit is not null AND idemptype = 1 AND tid = '".$param->info->employee."' AND work_date BETWEEN '".$date."' AND '".$date1."'  GROUP BY tid  ORDER BY CONCAT(tid,work_date) ASC";
}else if($ids == ''){
    $Qry->fields    = "(separationdate > '".$date1."' OR separationdate IS NULL) AND idunit is not null AND idemptype = 1 AND work_date BETWEEN '".$date."' AND '".$date1."'  GROUP BY tid  ORDER BY CONCAT(tid,work_date) ASC";
}else{
    $Qry->fields    = "(separationdate > '".$date1."' OR separationdate IS NULL) AND idunit is not null AND idemptype = 1 AND idunit in (".$ids.") AND  work_date BETWEEN '".$date."' AND '".$date1."'  GROUP BY tid  ORDER BY CONCAT(tid,work_date) ASC";
}
                    ...
                    if($row['absent'] != 0){
                        ...
                        if($row1['obtriptype'] == 1){
                           ...
                           if($row['defaultschedid'] == 4 || $row['defaultsched'] == 'Rest Day'){
                               if($row['holiday']){
                                    //DISABLED SINCE ALL FALL ON HOLIDAYS SHOULD BE CONVERTED TO CTO
