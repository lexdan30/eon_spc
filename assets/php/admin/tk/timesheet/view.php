    ...
    if(mysqli_num_rows($rs)>=1){
        while($row=mysqli_fetch_array($rs)){
            ...
            if (empty($row['shift_status'])) {
                ...
                $shift_info = getDateShiftDatas($con, $row['empID'], $shift_fieldid, $shift_field, $shift_fieldin, $shift_fieldout, $shift_fieldb, $row['work_date']);
 		...
 		"in2" 	                => $row['in2']? date('h:i a', strtotime($row['in2'])) :'',
                "out2" 	                => $row['out2']? date('h:i a', strtotime($row['out2'])) :'',
