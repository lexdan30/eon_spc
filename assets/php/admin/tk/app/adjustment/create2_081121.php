				...
				foreach( $arr_acct  as $kk=>$vv ){
					...
					foreach( $param['info']['leave_dates']  as $keys=>$value ){
						...
						if( adjustmentExists($con, $date, $idacct, $idtimeleavetype) ){
							...
							$time 	   = time();
							$docnumber = "AA".$idacct.strtotime( $date_create.$time ).$time.$ctr;
							$ctr++;
							$Qry 			= new Query();	
							$Qry->table 	= "tbltimeadjustment";
							$Qry->selected 	= "creator, docnumber, idacct, idshift, idtimetype, date, stime, ftime, hrs, date_create, id_payperiod, approver1, approver1_date, approver1_time, approver1_stat, stat";
							...
							if($checke){										
								...
								if( !empty( $timesheet_data ) ){
									if( addToBackuptimesheet($con,$timesheet_data) ){
										$return = updateTimesheetLate($con, json_decode (json_encode ($param), FALSE));
									}
								}
							}							
						}
