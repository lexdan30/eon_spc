...
$Qry->selected  = "tid,
                    ...
                    SUM( 
                        IF( (acthrs - excess  < 4 AND  acthrs - excess > 2  AND (location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW'))) AND NOT FIND_IN_SET(3, batchnum))
                            ...
                            AND (FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(date_in, ' ',  timein) AS DATETIME), CAST(CONCAT(date_out, ' ', timeout) AS DATETIME)) / 60) / 15)) * 15) < 240)
                            OR (location = 2  AND aaapprove IS NOT NULL AND FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(work_date, ' ',  aastime) AS DATETIME), CAST(CONCAT(work_date, ' ', aaftime) AS DATETIME)) / 60) / 15)) * 15) < 240)
                                ,1
                                ,0
                            )
                        )
                    ...
                    OR (location = 2  AND aaapprove IS NOT NULL AND FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(work_date, ' ',  IF( CAST(aastime AS TIME) < CAST(loginschedref AS TIME) ,loginschedref ,aastime)) AS DATETIME), CAST(CONCAT(work_date, ' ', IF( CAST(aaftime AS TIME) < CAST(logoutschedref AS TIME) ,aaftime ,logoutschedref)) AS DATETIME)) / 60) / 15)) * 15) < 480)
                            ,1
                            ,0
                        )  
                    )AS hallowance,
                    ...
                    IF( (acthrs - excess >= 8 AND (location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW')) ) AND NOT FIND_IN_SET(3, batchnum))
                        ...
                        OR (location = 2  AND aaapprove IS NOT NULL AND FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(work_date, ' ',  IF( CAST(aastime AS TIME) < CAST(loginschedref AS TIME) ,loginschedref ,aastime)) AS DATETIME), CAST(CONCAT(work_date, ' ', IF( CAST(aaftime AS TIME) < CAST(logoutschedref AS TIME) ,timeout ,logoutschedref)) AS DATETIME)) / 60) / 15)) * 15) >= 480)
                            ,1
                            ,0
                        )
                    )AS wallowance,
