...
if( !empty( $param->accountid ) ){
	...
	$data	 	= array(
        "status"		  => "success",        
        "emps"            =>  getEmp($con,$str),
		"countleaves"     =>  getEmpCountLeaves($con,  $str, $dFrom, $dTo),
        "pp"              => $str,
	);
...
function getCountsLEAVE($con,  $ids, $dFrom, $dTo){
	...
	$rs 			= $Qry->exe_SELECT($con);
	Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getCountsLEAVE');
	if( mysqli_num_rows($rs) >= 1 ){
...
function getEmpCountLeaves($con, $ids, $dFrom, $dTo){
	...
	$rs 			= $Qry->exe_SELECT($con);
	Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getEmpCountLeaves');
	if( mysqli_num_rows($rs) >= 1 ){
