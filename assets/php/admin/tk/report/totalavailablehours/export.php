...
$Qry->fields    ="absent NOT LIKE '0%'".$search;
      ...
      //  $date_format=date_create($row['date']);
		...
		$name23[] = array(
            /*
                    ...
                    $row['empname'],
                    $row['work_date'],
                     is_null($row['lvapprove'])?'AWOL':'LEAVE',
                     $row['absent']
		);
...
header('Content-Disposition: attachment; filename=TotalAvailableHoursReport_'.$date.'.csv');
$output = fopen('php://output', 'w');
//fputcsv($output, array($param['company']));
fputcsv($output, array("New World Makati Hotel"));
fputcsv($output, array("Total Available Hours Report"));
