...
UnitTest.asynctest('tinymce.plugins.paste.browser.ImagePasteTest', function () {
  ...
  suite.test('TestCase-TBA: Paste: paste data with script', function (editor) {
    editor.setContent('');
    editor.execCommand('mceInsertClipboardContent', false, { content: '<p><img src="non-existent.webp" onerror="alert(\'!\')" /></p>' });

    LegacyUnit.equal(editor.getContent(), '<p><img src="non-existent.webp" /></p>');
  });
