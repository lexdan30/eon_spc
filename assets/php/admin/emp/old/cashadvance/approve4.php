    ...
    if( !empty($param->accountid) ){
		...
		if( mysqli_num_rows($rs)==1 ){
			if( $row = mysqli_fetch_array($rs) ){
				...
				$Qrye->selected = " approver4		=	'".$param->accountid."',						
									approver4_date	=	'".$date."',
									approver4_time	=	'".$time."',
									approver4_status=	'1' ";
				
				if( (int)$approver_ctr == 4 ){					
				...
				if($checke){
					...
					*/
					...
					if( (int)$approver_ctr > 4 ){
						...
						$QryA->selected	=	"approver_5a, approver_5b";
						$QryA->fields	=	"idform = '".$param->form_id."'";
						$rsA			=	$QryA->exe_SELECT($con);
						if(mysqli_num_rows($rsA)>=1){
							if($rowA=mysqli_fetch_array($rsA)){
								if( !empty( getAccountEmail( $con, $rowA['approver_5a'] ) ) ){
									$recipients[] = array(
										getAccountEmail( $con, $rowA['approver_5a'] ) => getAccountName( $con, $rowA['approver_5a'] )
									);
								}

								if( !empty( getAccountEmail( $con, $rowA['approver_5b'] ) ) ){
									$recipients[] = array(
										getAccountEmail( $con, $rowA['approver_5b'] ) => getAccountName( $con, $rowA['approver_5b'] )
									);
								}
							}
						}
						...
						if(!empty($recipients)){
							...
							$mailBody .="<br />Entry has been approved by Approver 4.<br />Waiting for your approval.<br /><br />";
