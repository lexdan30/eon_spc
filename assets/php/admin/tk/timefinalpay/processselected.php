...
if(mysqli_num_rows($rs)>=1){
    while($row=mysqli_fetch_assoc($rs)){ 
        $prow['process_start'] = getPeriodcovered($con,$row);
        $prow['process_end'] = $row['sdate'];
        $prow['batchnum'] = $row['batchnum'];
        $prow['tid'] = $row['id'];
        $prow['release_date'] = $param->info->rdate;
        
        $data[] = processFinalpay($con,$prow);
...
function processFinalpay($con,$prow){
    $Qry = new Query();
    if($prow['batchnum'] == '5'){
        $Qry->table="vw_timesheetfinal_helper";
    }elseif($prow['batchnum'] == '3,4'){
    ...
    $Qry->selected  = "tid,
                    ...
                    SUM(ut) AS Undertime,
                    SUM(absent) AS `Abs`,
                    SUM( IF( (acthrs - excess)  + lateref > shifthrs, shifthrs,  (acthrs - excess)  + lateref ) ) AS WHrs,
                    ...
                    SUM(IF(leaveidtype = 6 AND leaveappstatus = 1   AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ) , `leave`, 0)) AS TC_CL,
                    ...
                    SUM(IF(leaveidtype = 8 AND leaveappstatus = 1   AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ) , `leave`, 0)) AS TC_BDL,
                    ...
                    SUM(IF(leaveidtype = 21 AND leaveappstatus = 1   AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ) , `leave`, 0)) AS TC_WL,
                    SUM(IF(leaveidtype = 22 AND leaveappstatus = 1   AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ) , `leave`, 0)) AS TC_MENSTL,
                    SUM(IF(leaveidtype = 23 AND leaveappstatus = 1   AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ) , `leave`, 0)) AS TC_CVL,
                    SUM((CASE 
                    ...
                    AS TC_RD,
                    SUM((CASE 
                        WHEN defaultschedid != 4 AND holidaytype = 'SPECIAL' AND idlvl NOT IN(1,2,3,4,5) THEN (acthrs - excess) + (IF(leaveidtype != 3 AND ( (lvapprove BETWEEN period_start AND period_endref) OR lvapprove < period_start ) , `leave`, 0))
                        ELSE 0
                        END))
                    AS H_SH,
                    SUM((CASE 
                    ...
                    AS otallowance,
                    SUM( 
                        IF( tid != 3367 AND (acthrs - excess + lateref  < 4 AND  acthrs - excess + lateref > 2  AND (location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW'))) AND NOT FIND_IN_SET(3, batchnum) AND obapprove IS NULL AND CAST(timein AS TIME) < CAST('17:00' AS TIME))
                            OR ((timein IS NOT NULL OR timeout IS NOT NULL OR timein != '' OR timeout != '') 
                            AND CAST(timein AS TIME) < CAST('17:00' AS TIME)
                            ...
                            AND rdacthrs < 4 AND rdacthrs > 2)
                            OR (location = 2 AND CAST(aastime AS TIME) < CAST('17:00' AS TIME)  AND location = 2  AND aaapprove IS NOT NULL AND aaapprove BETWEEN period_start AND period_endref  AND FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(aa, ' ',  aastime) AS DATETIME), CAST(CONCAT(aadate_out, ' ', aaftime) AS DATETIME)) / 60) / 15)) * 15) < 240)
                            AND empstat != 9  
                            ,1
                            ,0)
                    )
                    AS mallowance,
                    SUM(
                        IF(tid != 3367 AND  ((acthrs - excess +lateref  >= 4 AND  (acthrs - excess + lateref  < 8))  AND ( location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW')) ) AND NOT FIND_IN_SET(3, batchnum)  AND obapprove IS NULL AND CAST(timein AS TIME) < CAST('17:00' AS TIME))
                            OR ((timein IS NOT NULL OR timeout IS NOT NULL OR timein != '' OR timeout != '')
                            
                            AND CAST(timein AS TIME) < CAST('17:00' AS TIME)
                            AND (holiday IS NOT NULL OR DAYNAME(work_date) = 'Sunday'  OR DAYNAME(work_date) = 'Saturday') 
                            AND (location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW'))) AND NOT FIND_IN_SET(3, batchnum)
                            AND rdacthrs >= 4 AND rdacthrs < 8)
                            OR (location = 2 AND CAST(aastime AS TIME) < CAST('17:00' AS TIME)  AND aaapprove IS NOT NULL AND aaapprove BETWEEN period_start AND period_endref  AND FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(aa, ' ',  IF( CAST(aastime AS TIME) < CAST(loginschedref AS TIME) ,loginschedref ,aastime)) AS DATETIME), CAST(CONCAT(aadate_out, ' ', IF( CAST(aaftime AS TIME) < CAST(logoutschedref AS TIME) ,aaftime ,logoutschedref)) AS DATETIME)) / 60) / 15)) * 15) < 480)
                            AND empstat != 9
                            ,1
                            ,0
                        )  
                    ...
                    AS hallowance,
                    SUM(
                        IF(tid != 3367 AND (acthrs - excess + lateref >= 8 AND (location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW')) ) AND NOT FIND_IN_SET(3, batchnum) AND obapprove IS NULL AND CAST(timein AS TIME) < CAST('17:00' AS TIME))
                            OR ((timein IS NOT NULL OR timeout IS NOT NULL OR timein != '' OR timeout != '')
                            ...
                            AND (location = 2 OR FIND_IN_SET(tid, (SELECT `value` FROM tblpreference WHERE alias = 'MSALLOW'))) AND NOT FIND_IN_SET(3, batchnum)
                            AND rdacthrs >= 8)
                             OR (location = 2 AND CAST(aastime AS TIME) < CAST('17:00' AS TIME) AND aaapprove IS NOT NULL AND aaapprove BETWEEN period_start AND period_endref  AND FLOOR((((TIMESTAMPDIFF( SECOND,CAST(CONCAT(aa, ' ',  IF( CAST(aastime AS TIME) < CAST(loginschedref AS TIME) ,loginschedref ,aastime)) AS DATETIME), CAST(CONCAT(aadate_out, ' ', IF( CAST(aaftime AS TIME) < CAST(logoutschedref AS TIME) ,timeout ,logoutschedref)) AS DATETIME)) / 60) / 15)) * 15) >= 480)
                            AND empstat != 9
                            ,1
                            ,0
                        )
                    ...
                    AS swallowance,
                    SUM(shifthrs) AS Rhrs,
                        IF((SELECT isconvertible FROM `tblleaves` WHERE id = 1) = 'Y' 
                            ,(SELECT (entitle + carry_over - used)  FROM `tblaccountleaves` WHERE idacct = tid AND idleave = 1)
                            ,0
                        )
                    AS lc_sl,
                        IF((SELECT isconvertible FROM `tblleaves` WHERE id = 2) = 'Y' 
                            ,(SELECT (entitle + carry_over - used)  FROM `tblaccountleaves` WHERE idacct = tid AND idleave = 2)
                            ,0
                        )
                    AS lc_vl";


    $Qry->fields    = "tid = '". $prow['tid']."' AND work_date BETWEEN '". $prow['process_start'] ."' AND '".$prow['process_end']."' GROUP BY tid  ORDER BY CONCAT(tid,work_date) ASC";
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            deletets($con,$row['tid']);
            ...
            checkadjustments($con,$prow['process_start'],$prow['process_end'],$row,$lastid);
            finalpaystatus($con,$prow);
            return $row;
        } 
    }
}

function deletets($con,$empid){
    $Qry = new Query();	
    $Qry->table     = "tbltimesheetsummary";
    $Qry->fields    = "idacct='".$empid."' AND type = 'Final Pay'";
    $rs = $Qry->exe_DELETE($con);

    $Qry = new Query();	
    $Qry->table     = "tblfinalpay";
    $Qry->fields    = "idacct='".$empid."'";
    $rs = $Qry->exe_DELETE($con);
}
        ...
        if($row=mysqli_fetch_assoc($rs)){
            ...
            if(mysqli_num_rows($rs1)>= 1){
                if($row1=mysqli_fetch_assoc($rs1)){
                    $row1['leave'] = $row1['leave'] - $row['leavep'];
                               ...
                               if($row['holiday']){
                                ...
                                if($row['holiday']){
                                    if($row['holidaytype'] == 'Legal'){
                                        ...
                                        if($row['idlvl'] == '2' || $row['idlvl'] == '3' ){
                                            updateadjustmentscol($con,'adj_otallowance',$ot,$empid,$lastid);
                                        }else{
...
function finalpaystatus($con,$prow){
    $Qry           = new Query();
    $Qry->table    = "tblfinalpay";
    $Qry->selected = "idacct,start,end,release_date";
    $Qry->fields   = " '".$prow['tid']."',
                        '".$prow['process_start']."',
                        '".$prow['process_end']."',
                        '".$prow['release_date']."'
