...
(function (factory) {
  ...
  OrgChart.prototype = {
    ...
    nodeClickHandler: function (event) {
      ...
      getChildren(event.target.offsetParent.childNodes[1].innerText);
      //$('#modal-children').modal('show');
      //alert(event.target.offsetParent.childNodes[1].innerText);
      //console.log(event);
    },
    ...
    createNode: function (data) {
      ...
      if (opts.nodeTemplate) {
        ...
        $nodeDiv.append('<div class="title" style="font-size:6px">' + data[opts.nodeHead] + '</div>')
    ...
    exportPDF: function(canvas, exportFilename){
      ...
      //var docHeight = Math.floor(canvas.height);
       var docWidth = Math.floor(1000);
       var docHeight = Math.floor(500);

      // var docWidth = Math.floor((1000* 15) / 25.4);
      // var docHeight = Math.floor((500 * 15) / 25.4);
      ...
      doc.addImage(canvas.toDataURL(), 'png', 100, 100);
    ...
    exportPNG: function(canvas, exportFilename){
      ...
      if ((!isWebkit && !isFf) || isEdge) {
        window.navigator.msSaveBlob(canvas.msToBlob(), exportFilename + '.webp');
      } else {
        ...
        if (!$chartContainer.find(selector).length) {
          $chartContainer.append('<a class="oci-download-btn' + (that.options.chartClass !== '' ? ' ' + that.options.chartClass : '') + '"'
                                 + ' download="' + exportFilename + '.webp"></a>');
        }
    ...
    export: function (exportFilename, exportFileextension) {
      ...
      var flag = that.options.direction === 'r2l' || that.options.direction === 'l2r';
  ...
  var childrenlist = [];
  function getChildren(dept){

    
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.open( "GET",'/eon_spc/assets/php/admin/org/structure/getunit.php?id=' + dept, false ); // false for synchronous request
    xmlHttp.send( dept );
    let children = {};
    children = xmlHttp.responseText;
    let childs = eval(children);

    $("#modal-children .modal-contents").html("")
    if(childs[0].children.length != 0){
      for(var i=0; i<childs[0].children.length; i++){
        childrenlist.push({'id':childs[0].children[i].id,
                           'fullname': childs[0].children[i].fullname,
                           'empid': childs[0].children[i].empid,
                           'pic': childs[0].children[i].pic,
                           'post': childs[0].children[i].post});
                           //$("#modal-children .modal-contents").append(`<img  src=`+childs[0].children[i].pic+` style="margin-right: 5px; width: 30px; height: 30px; border-radius: 50%;" class="user-image mar0">` +" "+childs[0].children[i].fullname+`<br>`);
                           $("#modal-children .modal-contents").append(

                          `<table style="width:100%">
                           <tr style="border-bottom: 1px solid #dee2e6!important; width:100%">
                               <td style="padding:5px !important; width:10%">
                               <img  src=`+childs[0].children[i].pic+` style="margin-right: 5px; width: 40px; height: 40px; border-radius: 50%;" class="user-image mar0">
                              
                               </td>
                               <td style="padding:5px !important; width:100%">
                                
                                     <table>
                                         <tr>
                                             <td style="font-weight: 600 !important">  `+childs[0].children[i].empid+`
                                             `+childs[0].children[i].fullname+`
                                             </td>
                                             </tr>
                                          <tr>   
                                          <td style="color: red">
                                             `+childs[0].children[i].post+`
                                             </td>
        ...
        $("#modal-children .modal-title").html("Staffs under " +childs[0].name);
        $('#modal-children').modal('show');
    }
    else{
      //alert("No more staffs to display");
      $('#no-children').modal('show');
    }


    //console.log(childs[0].name);
    return xmlHttp.responseText;


    // var request = {
		// 	method: 'POST',
		// 	url: '/eon_spc/assets/php/admin/org/structure/getunit.php?id=' + dept,
		// 	data: dept,
		// 	headers: {
    ...
    //   }
		...
		// 	'idunit': dept
		// }
		// $http.requ('/eon_spc/assets/php/admin/org/structure/getunit.php',urlData)
