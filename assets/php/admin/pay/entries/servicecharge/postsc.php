...
$Qry->fields   = "'".$param->docnumber."',
                    ...
                    if( !empty( $param->info->totbpays ) ){
                        $Qry->selected 	= $Qry->selected . ", backpay_hrs";
                        $Qry->fields 	= $Qry->fields 	 . ",'". $param->info->totbpays."'";
                    }  
                    if( !empty( $param->info->prev_scrate ) ){
                        $Qry->selected 	= $Qry->selected . ", prev_hourlyrate";
                        $Qry->fields 	= $Qry->fields 	 . ",'". $param->info->prev_scrate."'";
                    } 
                    if( !empty( $param->info->totbp_amount ) ){
                        $Qry->selected 	= $Qry->selected . ", tot_bp_amount";
                        $Qry->fields 	= $Qry->fields 	 . ",'". $param->info->totbp_amount."'";
                    } 
                    if( !empty( $param->info->netsc ) ){
                        $Qry->selected 	= $Qry->selected . ", net_sc_amount";
                        $Qry->fields 	= $Qry->fields 	 . ",'". $param->info->netsc."'";
                    } 
    ...
    foreach($param->info->result as $key=>$value){
        ...
        $Qry1->fields   = "'".$servicechargeid."',
                        ...
                        if( !empty( $value->empbpays ) ){
                            $Qry1->selected 	= $Qry1->selected . ", backpay_hrs";
                            $Qry1->fields 	= $Qry1->fields 	 . ",'".$value->empbpays."'";
                        }  
                        if( !empty( $value->total_empbpay ) ){
                            $Qry1->selected 	= $Qry1->selected . ", backpay_amount";
                            $Qry1->fields 	= $Qry1->fields 	 . ",'".$value->total_empbpay."'";
                        }  
