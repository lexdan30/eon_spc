...
$Qry 			= new Query();	
$Qry->table     = "
(SELECT id, empid, sexstr, idunit, pic, lname, fname, mname, empname, post, etypeid, empstat, business_unit, superior, idsuperior FROM vw_dataemployees) AS de 
LEFT JOIN (SELECT tid AS empID, temp, leaveidtype AS idleave, leaveappstatus AS leavestat, work_date, fshfname AS shift_status,
idshift, stime AS shiftin, ftime AS shiftout, timein AS `in`, holiday_type, timeout AS `out`, timein2 AS `in2`, timeout AS `out2` FROM vw_mng_timesheetfinal) AS dt ON (de.id = dt.empID) 
