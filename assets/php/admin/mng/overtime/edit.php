...
if(!empty($param->accountid)){	
	...
	if( (int)$param->info->curr_appr == 1 ){ // approver 1
		...
		foreach( $param->info->data as $ka => $va ){
			...
			if($va->actual_hrs == '0.00' && $va->aa_approved == false && $param->info->stat != '2'){
				// $arr_err[] = array(
				// 	"id"	=> $va->id,
				// 	"date"	=> $va->date,
				// 	"name"	=> $va->empname,
				// 	"err"	=> "You can not approve this request if no approved adjustment"
				// );
				$return = json_encode(array("status"=>"noadj"));
				print $return;	
				mysqli_close($con);
				return;
			}
				...
				if($row=mysqli_fetch_array($rs2)){
					if( (($row['approver_1b'] == $idacct || $row['approver_1c'] == $idacct) && $va->idacct == $idacct) || 
						...
						if( !empty( $hrs ) ){
							...
							// if(getflexinfo($con,$va->idacct,$va->date)){
							// 	if($splice[1] >= 00 && $splice[1] < 25){
							// 		$hrs = $splice[0] . '.' . '00';
							// 	}elseif($splice[1] >= 25 && $splice[1] < 50){
							// 		$hrs = $splice[0] . '.' . '25';
							...
							// 	}
							// }else{
							// 	$hrs = getflexihours($con,$va->planned_start,$va->planned_end);
							// }
							//return print_r($hrs);
							$evaluate = 0;
							if($hrs != '0.00'){
							...
							if($va->eto_stat == 1){
								if($va->actual_hrs < 8){
									$hrs = 4.00;
								}else if($va->actual_hrs >= 8 && $va->actual_hrs < 12){
									$hrs = 8.00;
								}else if($va->actual_hrs >= 12 && $va->actual_hrs < 16){
									$hrs = 12.00;
								}else if($va->actual_hrs >= 16 && $va->actual_hrs < 20){
									$hrs = 16.00;
								}else if($va->actual_hrs >= 20 && $va->actual_hrs < 24){
									$hrs = 20.00;
								}else if($va->actual_hrs >= 24 && $va->actual_hrs < 28){
									$hrs = 24.00;
								}else{
									$hrs = 0.00;
								}
...
function getflexihours($con,$planned_start,$planned_end){
	$start = strtotime($planned_start);
	$end = strtotime($planned_end);
