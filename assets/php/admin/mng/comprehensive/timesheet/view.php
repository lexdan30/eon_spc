...
//$Qry->fields    = "etypeid=1 AND id != '".$param->accountid."' AND (idsuperior='".$param->accountid."' or idunit IN (".$ids.")) ORDER BY empname ". $search . " LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize;
//$Qry->fields    = "id != '".$param->accountid."' AND (idsuperior='".$param->accountid."' or idunit IN (".$ids.")) AND sched is not null AND etypeid=1 ORDER BY empname". $search . " LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize;
$Qry->fields    = "etypeid=1 AND id != '".$accountid."' AND (idsuperior='".$accountid."') ". $search . " ORDER BY empname  LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize;
...
if(mysqli_num_rows($rs)>= 1){
    ...
    while($row=mysqli_fetch_assoc($rs)){
        ...
        $myData = array('status' => 'success', 'result' => $data, 'totalItems' => getTotal($con , $where,$accountid), 'fields' => $Qry->fields);
...
function getVwTimesheet($con, $idacct, $date, $date1){
    ...
    $Qry->selected="
    work_date, `name`, holidaytype, timein, timeout,
    timein2, timeout2, stime, ftime, sstime,
    sftime, date_in, date_out, date_in2, date_out2,
    `late`, ut, absent, np, shifthrs,
    acthrs, excess, othrs, tid, cs,
    csstatus, csname, shifttype, aastatus, aastime,
    aaftime, aasstime, aasftime, otstatus, leaveappstatus,
    leavename, `leave`, obtripstatus, obtrip, obstart,
    obend, csapprove, npot, holiday, defaultsched,
    alias, period_start, period_endref, aaapprove, otapprove,
    obapprove, lvapprove, leaveend, leavestart, lateref, utref, idtimeleavetype, leaveidtype";
    $Qry->fields=" tid='".$idacct."' AND work_date BETWEEN '".$date."' AND '".$date1."' ORDER BY work_date ASC";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getVwTimesheet');
    if(mysqli_num_rows($rs)>=1){
        while($row=mysqli_fetch_assoc($rs)){
            ...
            if($row['csstatus'] == 1 ){
                if($row['name'] == '<p class="csuccess fw9">RD</p>'){
                    ...
                    if( (($row['csapprove'] >= $periodstart) && ($row['csapprove'] >= $periodend)) ){
                        $row['name'] = $row['name'];
                    }else{
...
function getDateShiftDatas($con, $idacct, $idshift, $cols, $in, $out, $b, $date){
    ...
    $rs            = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getDateShiftDatas');
    if (mysqli_num_rows($rs) >= 1) {
        ...
        Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getDateShiftDatas(Qry2)');
...
function getTotalApprovedCS($con, $date, $date1, $idacct){
    ...
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalApprovedCS');
    if(mysqli_num_rows($rs)>=1){
...
function getTotalApprovedAA($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=1 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalApprovedAA');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalApprovedOT($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=1 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalApprovedOT');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalApprovedLV($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=1 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalApprovedLV');
    if(mysqli_num_rows($rs)>=1){            
        return mysqli_fetch_assoc($rs)['ctr'];
...
function getTotalApprovedOB($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=1 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalApprovedOB');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalDeclinedCS($con, $date, $date1, $idacct){
    ...
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalDeclinedCS');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalDeclinedAA($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=2 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalDeclinedAA');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalDeclinedOT($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=2 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalDeclinedOT');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalDeclinedLV($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=2 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalDeclinedLV');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalDeclinedOB($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=2 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalDeclinedOB');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalPendingCS($con, $date, $date1, $idacct){
    ...
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalPendingCS');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalPendingAA($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=3 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalPendingAA');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalPendingOT($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=3 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalPendingOT');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalPendingLV($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=3 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalPendingLV');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotalPendingOB($con, $date, $date1, $idacct){
    ...
    $Qry->fields="stat=3 AND date BETWEEN '".$date."' AND '".$date1."' AND idacct='".$idacct."' AND cancelby IS NULL";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotalPendingOB');
    if(mysqli_num_rows($rs)>=1){
        return mysqli_fetch_assoc($rs)['ctr'];
    }
    return '';
...
function getTotal($con,$search){
    ...
    $rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getTotal');
		if(mysqli_num_rows($rs)>= 1){
...
function getlateapproveChangeShift($con ,$date , $work_date, $idacct){
    ...
    $Qry=new Query();
    $Qry->table="
    (SELECT `date`, idshift, idacct, stat, approver1_date, id FROM tbltimeshift) as cs LEFT JOIN
    (SELECT id, `name` FROM tblshift) as ts ON (cs.idshift = ts.id) LEFT JOIN
    (SELECT idlvl, idacct FROM tblaccountjob) as aj ON (cs.idacct = aj.idacct)";
    $Qry->selected="`date`, ts.name";
...
function getlateapproveOvertime($con ,$date , $work_date, $idacct){
    $data = array();
    $Qry=new Query();
    $Qry->table="(SELECT id, idacct, stat, hrs, approve_hr, planhrs, `date`, approver2_date, approver1_date FROM tbltimeovertime) as app LEFT JOIN (SELECT idacct, idlvl FROM tblaccountjob) as aj ON (app.idacct = aj.idacct)";
    $Qry->selected="hrs, approve_hr, planhrs, `date`";
...
function getlateapproveTimesheetLeaves($con ,$date , $work_date, $idacct){
    $data = array();
    $Qry=new Query();
    $Qry->table="(SELECT idleave, `date`, id, idacct, stat, date_approve FROM `tbltimeleaves`) as lv LEFT JOIN (SELECT id, `name` FROM tblleaves) as lvs ON (lv.idleave = lvs.id) LEFT JOIN (SELECT idacct, idlvl FROM tblaccountjob) AS aj ON (lv.idacct = aj.idacct)";
    $Qry->selected="idleave, `date`, lvs.name";
