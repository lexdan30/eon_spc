...
if( !empty( $param->accountid ) && !empty($str) ){
	...
	$Qry2->selected  = "empname, COUNT(tid) AS ctr";
	$Qry2->fields    = "tid IN (".$str.") AND absent IS NOT NULL AND absent <> '0' AND work_date BETWEEN '".$first_day_this_month."' AND '".$last_day_this_month."' GROUP BY tid ORDER BY COUNT(tid) DESC LIMIT 5 ";
