...
*********************************TRUNCATE QUERY STATEMENT*******************************/	
	...
	function getlastBatchid($con){
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return getlastBatchidCount($con, $row['batchid']);
			}
		}
		return 0;
	}

	function getlastBatchidCount($con,$batchid){
		$Qry = new Query();	
		$Qry->table     = "tbltimelogs";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND batchid= '".$batchid."'";
		$rs = $Qry->exe_SELECT($con);
	...
	function getUnitHeads($con){
		...
		$Qry->fields    = "id>0 order by fname";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
	...
	function getMainDepartment($con){
		...
		$Qry = new Query();	
		$Qry->table     = "tblsites";
		$Qry->selected  = "id, sites";
		$Qry->fields    = "id>0 AND id <> 6 order by sites";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
	...
	function getAccounts($con, $id){
		...
		$Qry->selected  = "id, idaccttype, empid, empname, idsuperior, superior, idpos, post";
		$Qry->fields    = "id!=1 AND etypeid=1 ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				if(!empty($row['email'])){
	...
	function getJAccounts($con, $id){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "id, idaccttype, empid, empname, empid, idsuperior, superior, idpos, idunit, post";
		$Qry->fields    = "etypeid=1 AND empid LIKE 'KPIJ%' ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				if(!empty($row['email'])){
	...
	function getUnderAccounts($con, $id){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "id, idaccttype, empid, empname, idsuperior, superior, idpos, post, email";
		$Qry->fields    = "etypeid=1 AND idsuperior='".$id."' AND id <> '".$id."' ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
	...
	function getAccountsOrg($con, $id){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "id, idaccttype, empid, empname, empid, idsuperior, superior, idpos, idunit, post";
...
function getAllAccounts($con, $id){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "id, idaccttype, empid, empname, empid, idsuperior, superior, idpos, idunit, post";
		$Qry->fields    = "id!=1 AND etypeid IN(1,2,3) ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
	...
	function getWShifts($con){
		...
		$Qry->selected  = "id, name, idsun,
		sun, sun_in, sun_out, sun_brk,
		mon, mon_in, mon_out, mon_brk,
		tue, tue_in, tue_out, tue_brk,
		wed, wed_in, wed_out, wed_brk,
		thu, thu_in, thu_out, thu_brk,
		fri, fri_in, fri_out, fri_brk,
		sat, sat_in, sat_out, sat_brk";
		$Qry->fields    = "id>0";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
	...
	function getJobLvls($con){
		...
		$Qry = new Query();	
		$Qry->table     = "tbljoblvl";
		$Qry->selected  = "id, alias, lvl, `type`";
	...
	function getPosition($con, $id){
		$Qry = new Query();	
		$Qry->table     = "vw_data_assignposition";
		$Qry->selected  = "*";
		$Qry->fields    = "idunit='".$id."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['name'];
	...
	function getPositionEmp($con, $id){
		$Qry = new Query();	
	...
	function getHeadPicName($con,$id,$unit){
		$Qry = new Query();	
		$Qry->table     = "tblaccount";
		$Qry->selected  = "*";
		$Qry->fields    = "id = '".$id."'";
		...
		if(mysqli_num_rows($rs)>= 1){
			...
			if($row=mysqli_fetch_assoc($rs)){
				...
				if($row['idemptype'] == 3){
					return  '<img src="assets/php/admin/hr/employee/pix/undefined.webp?1591229368" style="margin-right: 5px; width: 30px; height: 30px; border-radius: 50%;" class="user-image mar0"><label style="color:yellow;">Vacant Position</label><br>'.getPositionEmp($con, $id);
				}
				else{
					return  '<img src="assets/php/admin/hr/employee/pix/'.$row['pic'].'?1591229368" style="margin-right: 5px; width: 30px; height: 30px; border-radius: 50%;" class="user-image mar0">'.$row['empid'].' '.$row['lname'].", ". $row['fname']." ". $row['mname'].'<br>'.getPositionEmp($con, $id);
				}
			}
		}		
		return  '<img src="assets/php/admin/hr/employee/pix/undefined.webp?1591229368" style="margin-right: 5px; width: 30px; height: 30px; border-radius: 50%;" class="user-image mar0"><label style="color:yellow;">Vacant Position</label><br>No Assign Head';
	}

	function getCompanyPic($con){
		$data = array();
		$Qry = new Query();	
		...
		if(mysqli_num_rows($rs)>= 1){
				if($row=mysqli_fetch_assoc($rs)){
					return  ' <img src="assets/php/admin/org/company/'.$row['id'].'.webp?1591229368" style="width: 96%; overflow:hidden; height: 30px;" class="user-image mar0">';
				}		
	}
		return '';
	}
	...
	function getShiftsTypes($con){
		...
		$Qry = new Query();	
		$Qry->table     = "tblshift";
		$Qry->selected  = "id, name, stime, ftime, break";
		$Qry->fields    = "id>0 ORDER BY name";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
			...
			if($row=mysqli_fetch_assoc($rs)){
				...
				if( (int)$idtimeleavetype == 1 ){
					...
					if((int)$idshift == 89 || (int)$idshift == 93){
						$ftime = date('H:i:s', strtotime($stime.'+4 hour'));
					}else{
						$ftime = $row['breakin'];
					}
				}elseif( (int)$idtimeleavetype == 3 ){
					if((int)$idshift == 89 || (int)$idshift == 93){
						$stime = date('H:i:s', strtotime($row['stime'].'+4 hour'));
					}else{
						$stime = $row['breakout'];
					}
					$ftime = $row['ftime'];
	...
	function getRegions($con){
		...
		$Qry = new Query();	
		$Qry->table     = "tblregion";
		$Qry->selected  = "id, regDesc, regCode";
		$Qry->fields    = "id>0";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
	...
	function getProvinces($con){
		...
		$Qry->selected  = "id, regCode, provDesc, provCode";
		$Qry->fields    = "id>0";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
	...
	function getMunicipality($con){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "tblmunicipality";
		$Qry->selected  = "id, regDesc, provCode, citymunCode, citymunDesc";
		$Qry->fields    = "id>0";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
	...
	function checkHolidayNames($con, $name, $id ){
		$Qry = new Query();	
		$Qry->table     = "tblholidays";
		$Qry->selected  = "*";
		$Qry->fields    = "name = '".$name."'";
	...
	function getCalendarSched($con,$id,$date,$cols){
		$data = array();	
		$Qry = new Query();	
		...
		$Qry->selected  = "da.wshift";
		$Qry->fields    = "de.id='".$id."' AND dt.work_date = '".$date."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				$wshift = $row['wshift'];
				...
				$col = explode(",",$cols);
				$Qry2 = new Query();	
				$Qry2->table     = "vw_datacalendar";
				$Qry2->selected  = $cols;
				$Qry2->fields    = "id='".$wshift."'";
				$rs2 = $Qry2->exe_SELECT($con);
				if(mysqli_num_rows($rs2)>= 1){			
					if($row2=mysqli_fetch_assoc($rs2)){
	...
	function getLatePayPeriod($con, $dateApprove){
		...
		$Qry->table     = "tblpayperiod";
		$Qry->selected  = "*";
		$Qry->fields    = "period_start <= '".$dateApprove."'  AND  period_end >= '".$dateApprove."' ORDER BY pay_date ASC limit 1";		
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data = array( 
	...
	function getIdPeriod($con, $from, $to){
		$Qry = new Query();	
		$Qry->table     = "tblpayperiod";
		$Qry->selected  = "*";
	...
	function getPayPeriod( $con, $id_period=NULL ){
		...
		$Qry->selected  = "id, period_start, period_end, pay_date, hascontri, stat, tkstatus, `type`, tkprocess, payprocess, pay_period";
		if( empty( $id_period ) ){
			$Qry->fields    = "tkstatus=0 ORDER BY pay_date,type DESC limit 1";
		}else{
	...
	function checktklock($con,$date){// AD
		$Qry = new Query();	
		$Qry->table ="tblpayperiod";
		$Qry->selected ="id, period_start, period_end, pay_date, grace_hour, hascontri, tkstatus,stat"; 
		$Qry->fields ="tkstatus=1 AND stat=0 AND DATE(period_start) <= DATE('".$date."') AND DATE(period_end) >= DATE('".$date."') ORDER BY id ASC LIMIT 1";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			return true;
		}
	...
	function getPayPeriodts( $con, $id_period=NULL ){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "tblpayperiod";
		$Qry->selected  = "id, period_start, period_end, pay_date, grace_hour, hascontri, stat, tkstatus, period_type";
		if( empty( $id_period ) ){
			$Qry->fields    = "tkstatus=0 ORDER BY id ASC limit 1";
		}else{
			$Qry->fields    = "id=".$id_period;
		}
		$rs = $Qry->exe_SELECT($con);
	...
	function getLocationsbunits($con,$loc){
		$data = array();	
		if($loc == 2)
		{
			$locs = "site IN (2,6)";
	...
	function getLeaves($con){
		...
		$Qry->selected  = "id, name, hours, idtype, `type`, isconvertible, idactive";
	...
	function getLeaveBalance($con, $idacct, $idleave ){
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['balance'] - $row['pending'];
			}
		}
		return 0;
	}
	...
	function getOtApplication($con, $work_date, $acct_id, $start, $end){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "DATE(date)='".$work_date."' AND CAST(idacct AS INT) ='".$acct_id."' AND stat=1 /*AND date_approve BETWEEN '".$start."' AND '".$end."'*/ ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data = array( 
	...
	function getAllDepartment($con){
		...
		$Qry = new Query();	
		$Qry->table     = "vw_databusinessunits";
		$Qry->selected  = "id, name, alias, costcenter, idhead, idunder, unittype, isactive, stype, shead, stat, site";
		$Qry->fields    = "id>0 order by name";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
	...
	function getAllDepartmentTK($con){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_databusinessunits";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 GROUP BY name order by name";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
	...
	function getAllMainDepartment($con){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_databusinessunits";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND unittype = '3' order by name";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
	...
	function checkApplication($con, $acct, $date, $tbl){
		...
		$Qry->table     = $tbl;
		$Qry->selected  = "*";
		$Qry->fields    = "date='".$date."' AND idacct='".$acct."' AND stat NOT IN (2,4)";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return true;
    ...
    function getSumInfoabsent($con,$pay_period,$acct_id,$col){
		$Qry = new Query();	
		$Qry->table     = "vw_data_timesheet";
		$Qry->selected  = "IFNULL(SUM(".$col."),0) as ctr";
		$Qry->fields    = "empID='".$acct_id."' AND work_date BETWEEN '".$pay_period['pay_start']."' AND '".$pay_period['pay_end']."' AND adj_status != 'APPROVED'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){ 				
				return $row['ctr'];
	...
	function transAddBase($con){
		return "'58','16','1','2','65','7','8','21','5','6','19','3','4','17','9','10','20','11','12','18','13','14','64','22','23','63','42','43','48','120','25','67'";
	}
	
	function transDeductions($con){
		return "'33','34','40','121','38','39'";
	}
	...
	function getSSSCont($con,$sal){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "sal_creditfrom  <= ".$sal." AND yr_use='2021-01-01' ORDER BY id DESC LIMIT 1";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data = array(
					"id" 		=> $row['id'],
					"credit"	=> $row['sal_creditfrom'],
					"sssemp"	=> $row['empcont'],
					"ssscom"	=> $row['emprcont'],
                    "sssecc"	=> $row['ecc_er'],
                    "mfundemp"	=> $row['mandatory_ee'],
					"mfundcomp"	=> $row['mandatory_er']
				);
			}
		}
		return $data;
	...
	function getIBIGCont($con,$sal){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "sal <= ".$sal." AND yr_use='2021-01-01' ORDER BY id DESC LIMIT 1";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data = array(
	...
	function getHLTHCont($con,$sal){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "salfrom <= ".$sal." AND yr_use='2021-01-01' ORDER BY id DESC LIMIT 1";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data = array(
	...
	function getWITHTAX($con,$sal,$filter){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "mini <= ".$sal." AND stat = '".$filter."' AND yr_use='2021-01-01' ORDER BY id DESC LIMIT 1";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data = array(
	...
	function getLeaveUsed($con, $idacct, $idleave){
		...
		$Qry->selected  = "IF(SUM(hrs) IS NULL, 0 ,SUM(hrs))  AS ctr";
	...
	function getShiftIDData($con, $idshift){
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				$data = array($row['id'],$row['name'],$row['stime'],$row['ftime'],$row['break'],$row['reghrs'],$row['stype'],$row['sstime'],$row['sftime']);
			}
		}
		return $data;
	}

	function getActLogs($con, $id, $date){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "tbltimesheet";
		$Qry->selected  = "*";
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				$data = array($row['id'],$row['idshift'],$row['timein'],$row['timeout'],$row['timein2'],$row['timeout2']);
			}
		}
	...
	function getDBName($con){
		$Qry 			= new Query();	
		$Qry->table     = "tbldb";
		$Qry->selected  = "id, dbname, company, alias, iscorp";
		$Qry->fields    = "isorig='1'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
	...
	function getBirthDates( $con, $date_s ){
		...
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "pic, empid, fname, lname, suffix";
	...
	function getBirthdate($con, $idacct){
		$curyear = SysDate();
		$Qry 			= new Query();	
		$Qry->table     = "tblaccount";
		$Qry->selected  = "CONCAT(DATE_FORMAT(NOW(),'%Y-'),DATE_FORMAT(bdate,'%m-%d')) AS birthday";
		$Qry->fields    = "id='".$idacct."'";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
	...
	function getmultiNames($con, $idaccts){
		$names = array();
		$Qry=new Query();
		$Qry->table="vw_dataemployees";
		$Qry->selected="empname";
	...
	function getapprovers( $con, $idunder ){
		$data = array();
		$Qry 			= new Query();	
		$Qry->table     = "tblbunits";
		$Qry->selected  = "id, name, alias, costcenter, idhead, deputy1, deputy2, idunder, unittype";
	...
	function getapprover( $con, $idunder, $idacct ){
		$data = array();
		$Qry 			= new Query();	
		$Qry->table     = "tblbunits";
		$Qry->selected  = "*";
		...
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				if((int)$idacct === (int)$row['idhead']){
					$data = getupperapprover($con, $row['idunder'], $row['idhead']);
				}else{
					$data = array(
						'id' 			=> $row['id'],
						'unit'			=> $row['name'],
	...
	function getupperapprover( $con, $idunder, $idacct ){
	...
	function getclass( $con, $idpos, $idloc){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_data_assignposition";
		$Qry->selected  = "*";
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				...
				foreach($unitids as $key => $unit){
					...
					$Qry2->selected  = "*";
					$Qry2->fields    = "id='".$unit."' AND site='".$idloc."'";
					$rs2 = $Qry2->exe_SELECT($con);
					if(mysqli_num_rows($rs2)>= 1){
					...
					if(mysqli_num_rows($rs2)>= 1){
						if($row2=mysqli_fetch_assoc($rs2)){
							$data[] = array( 
								//"id"    => $row2['id'],
								"idsite"    => $row2['site'],
								"sitename"    => getLocName($con,$row2['site'])
							);
						}
					}
				}
	...
	function getChildNodes($arr_id, $arr ){
		foreach( $arr as $vv ){
			array_push( $arr_id, $vv["id"] );
			if( !empty( $vv["nodechild"] ) ){
				getChildNodes( $arr_id, $vv["nodechild"] );
			}
		}
		return $arr_id;
	}
	...
	function getDepartmentUnit($con, $idunit){
		//lex get main idunit
		$iddept = '';
		$Qry = new Query();	
		$Qry->table     = "tblbunits";
		...
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){	
				...
				if($unittype === 3 ||  $unittype < 3){
					...
					$iddept = getDepartmentUnit( $con, $row['idunder'] );
				}
			}
		}
		...
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
					...
					"iddef"			=>  '1',
					"firsthalf" 	=>	array("id"=> (int)$row['firsthalf'],"checked"=> (int)$row['firsthalf']==0 ? false : true ),
					"secondhalf" 	=>	array("id"=> (int)$row['secondhalf'],"checked"=> (int)$row['secondhalf']==0 ? false : true ),
					"gl" 			=>	array("id"=> (int)$row['gl'],"checked"=> (int)$row['gl']==0 ? false : true )
				);
			}
		}else{
			$data = array(
	...
	function getAccountAllowance2( $con ){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblclasstrans";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND name LIKE '%allowance%' OR alias LIKE '%allowance%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getUploaderTemplate($con){
		...
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND template NOT LIKE '%Entries%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getPMUploaderTemplate($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tbluploader";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND template LIKE '%Entries%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getBatchEntriesName($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblclasstrans";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND entrytype LIKE '%B%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getLoanName($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblclasstrans";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND name LIKE '%Loan%' OR idclass=65";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getRecurringName($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblclasstrans";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND entrytype LIKE '%R%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getScName($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tblclasstrans";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND name LIKE '%Service Charge%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getEmpname($con){
		$data = array();	
		$Qry = new Query();	
		$Qry->table     = "vw_dataemployees";
		$Qry->selected  = "*";
		$Qry->fields    = "etypeid IN(1,2,3) and id != 1  ORDER BY empname ASC ";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array( 
	...
	function getHRUploaderTemplate($con){
		$data			= array();
		$Qry 			= new Query();	
		$Qry->table     = "tbluploader";
		$Qry->selected  = "*";
		$Qry->fields    = "id>0 AND template LIKE '%Increase%'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getPromotionApprover($con, $idacct){
		...
		$rs 			= $Qry->exe_SELECT($con);
		return mysqli_num_rows($rs) >= 1 ? 1 : 0;
	}
	...
	function getDeputy2($con, $idacct){
		...
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				...
				//na igo
				$arr_unit = getapprovers( $con, $row['idunit'] );
	...
	function getTimeLogsConf($con, $id=NULL){
		...
		$Qry->selected  = "id, alias, emp, wdate, wtime, atype, aout, ain, bout, bin, descript, note";
		$Qry->fields    = "id>0";
		if( !empty($id) ){
			$Qry->fields    = "alias='".$id."'";
		}
	...
	function getHoliday($con){
		...
		$Qry = new Query();
		$Qry->table = "tblholidays";
		$Qry->selected = "id, name, `date`, idtype";
		$Qry->fields = "id>0 AND YEAR(date) = YEAR(CURDATE()) order by name";
		$rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			while($row=mysqli_fetch_assoc($rs)){
				$data[] = array(
	...
	function getchartsofaccounts($con){
        $data = array();
        $Qry = new Query();
	...
	// Payroll Data Uploader
	// -- For Batch Entry
	function checkDupBatchEntryDocumentNo($con,$docnum){
		$Qry 			= new Query();	
		$Qry->table     = "tblbatchentries";
		$Qry->selected  = "docnum";
		$Qry->fields    = "docnum = '".$docnum."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
					return true;
	...
	function checkBatchEntryransaction($con, $transid, $paydate){
		$Qry 			= new Query();	
		$Qry->table     = "tblbatchentries";
		$Qry->selected  = "docnum";
		$Qry->fields    = "transactionid = '".$transid."' AND paydate = '".$paydate."' ";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['docnum'];
	...
	function checkBatchEntryDetailsExist($con, $batchid, $idacct){
		$Qry 			= new Query();	
		$Qry->table     = "tblbatchentriesdetails";
		$Qry->selected  = "*";
		$Qry->fields    = "batchentriesid = '".$batchid."' AND empid = '".$idacct."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['id'];
	...
	function getbatchID($con,$docnum){
		$Qry 			= new Query();	
		$Qry->table     = "tblbatchentries";
		$Qry->selected  = "id";
		$Qry->fields    = "docnum = '".$docnum."'";
	...
	//recurring entry
	function checkDupRecurringEntryDocumentNo($con,$docnum){
		$Qry 			= new Query();	
	...
	function checkRecurringEntryransaction($con, $transid, $total, $sdate, $edate){
		$Qry 			= new Query();	
		$Qry->table     = "tblrecurring";
		$Qry->selected  = "docnum";
		$Qry->fields    = "payitemid = '".$transid."' AND total = '".$total."' AND sdate = '".$sdate."' AND edate = '".$edate."' ";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['docnum'];
	...
	function getrecurringID($con,$docnum){
		$Qry 			= new Query();	
		$Qry->table     = "tblrecurring";
		$Qry->selected  = "id";
		$Qry->fields    = "docnum = '".$docnum."'";
	...
	function checkRecurringEntryDetailsExist($con, $recid, $idacct){
		$Qry 			= new Query();	
		$Qry->table     = "tblrecurringdetails";
		$Qry->selected  = "*";
		$Qry->fields    = "recurringid = '".$recid."' AND empid = '".$idacct."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['id'];
	...
	// SC Entry
	function checkDupSCDocumentNo($con,$docnum){
		$Qry 			= new Query();	
		$Qry->table     = "tblservicecharge";
		$Qry->selected  = "docnumber";
		$Qry->fields    = "docnumber = '".$docnum."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
					return true;
	...
	function checkSCtransaction($con, $payitemid, $releasedate){
		$Qry 			= new Query();	
		$Qry->table     = "tblservicecharge";
		$Qry->selected  = "docnumber";
		$Qry->fields    = "payitemid = '".$payitemid."' AND releasedate = '".$releasedate."' ";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
	...
	function getSCCount($con,$id){
		$Qry 			= new Query();	
		$Qry->table     = "tblservicecharge";
		$Qry->selected  = "headcount";
		$Qry->fields    = "id = '".$id."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
	...
	function checkSCDetailsExist($con, $scid, $idacct){
		$Qry 			= new Query();	
		$Qry->table     = "tblservicechargedetails";
		$Qry->selected  = "*";
		$Qry->fields    = "servicechargeid = '".$scid."' AND empid = '".$idacct."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['id'];
	...
	//Employee Loand
	function checkDupLoanDocumentNo($con,$docnum){
		$Qry 			= new Query();	
		$Qry->table     = "tblloans";
		$Qry->selected  = "docnumber";
		$Qry->fields    = "docnumber = '".$docnum."'";
	...
	function checkEmploanstransaction($con, $empid, $loanid, $loandate){
		$Qry 			= new Query();	
		$Qry->table     = "tblloans";
		$Qry->selected  = "docnumber";
		$Qry->fields    = "empid = '".$empid."' AND loanid = '".$paydate."' AND loandate = '".$loandate."' ";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['docnum'];
	...
	// Bonuses
	function checkDupBonusesDocumentNo($con,$docnum){
		$Qry 			= new Query();	
	...
	function checkBonusestransaction($con, $transid, $paydate){
		$Qry 			= new Query();	
		$Qry->table     = "tblbonuses";
		$Qry->selected  = "docnum";
		$Qry->fields    = "description = '".$transid."' AND releasedate = '".$paydate."' ";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['docnum'];
	...
	// Bonuses Details
	function getbonusID($con,$docnum){
		$Qry 			= new Query();	
		$Qry->table     = "tblbonuses";
		$Qry->selected  = "id";
		$Qry->fields    = "docnum = '".$docnum."'";
	...
	function getbonusTotal($con,$id){
		$Qry 			= new Query();	
		$Qry->table     = "tblbonuses";
		$Qry->selected  = "unitamount";
		$Qry->fields    = "id = '".$id."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
	...
	function checkBonusDetailsExist($con, $bonusid, $idacct){
		$Qry 			= new Query();	
		$Qry->table     = "tblbonusesdetails";
		$Qry->selected  = "*";
		$Qry->fields    = "bonusid = '".$bonusid."' AND idacct = '".$idacct."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['id'];
	...
	//wageincrease
	function checkWageIncreaseExist($con, $salary, $efdate,$idacct){
		$Qry 			= new Query();	
		$Qry->table     = "tblacctsalary";
		$Qry->selected  = "*";
		$Qry->fields    = "salary = '".$salary."' AND idacct = '".$idacct."' AND effectivity_date = '".$efdate."'";
		$rs 			= $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
			if($row=mysqli_fetch_assoc($rs)){
				return $row['id'];
