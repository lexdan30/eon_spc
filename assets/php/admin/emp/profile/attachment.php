...
if(!empty($param['accountid'])){
	...
	if( array_key_exists('file',$_FILES) ){
		$valid_formats = array("pdf", "png", "jpeg", "jpg");	
	...
	if( array_key_exists('file',$_FILES) ){
		...
		$save_name	 	= $param['refno'].'.'.$extMove;
		$folder_path 	= $param['targetPath'];
		move_uploaded_file($_FILES["file"]["tmp_name"], $folder_path.$save_name);
		$Qry33           = new Query();
		$Qry33->table    = "tblchangereq";
		$Qry33->selected = "attachment='".$save_name."'";
		$Qry33->fields   = "ref_num='".$param['refno']."'";                        
