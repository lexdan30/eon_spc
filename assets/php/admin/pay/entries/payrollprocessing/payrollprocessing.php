...
$_SESSION['id'] = $idpayperiod['period']['id'];
...
if( !empty($param->accountid) ){
    ...
    $Qry->selected  = "*";
    $Qry->fields = "idpayperiod ='".$id_paydate."' AND type ='".$idpayperiod['period']['type']."' AND idpaygrp ='".$paygroup."'";
	...
	if(mysqli_num_rows($rs)>=1){
		while($row=mysqli_fetch_assoc($rs)){
            ...
            // }
            if($row['idpaygrp'] == 8 || $row['idpaygrp'] == 7){
                processMonthlies($con,$row,$id_paydate);
            }
        }
        
        updatepayprocess($con, $param);
        $return = json_encode(array('status'=>'success', 'transactions' =>getreturn($con,$id_paydate,$paygroup) ));
    }else{
...
function processDalies($con,$row,$id_paydate){
    ...
    if($peraccount['sss_deduct1'] == 1 AND $peraccount['sss_deduct2'] == 1 ) {
        $sssdivisor = 2;
    }
    if($peraccount['ibig_deduct1'] == 1 AND $peraccount['ibig_deduct2'] == 1 ) {
        $pagibigdivisor = 1;
    }
    if($peraccount['health_deduct1'] == 1 AND $peraccount['health_deduct2'] == 1 ) {
        $philhealthdivisor = 1;
    }
    ...
    if($peraccount['ibig_deduct1'] == 1 OR $peraccount['ibig_deduct2'] == 1 ) {
        if($_SESSION['period_type'] == '1H' AND $peraccount['ibig_deduct1'] == 1){
            pagibigContribution($con ,$lastid ,grossContributionpi($con,$lastid),$peraccount['ibig_amt'],0,$pagibigdivisor);
        }
        ...
        if($_SESSION['hascontri'] == 2){
            pagibigContribution($con ,$lastid ,grossContributionpi($con,$lastid),0,1);
        }
    }
...
function processMonthlies($con,$row,$id_paydate){
    ...
    if($peraccount['ibig_deduct1'] == 1 OR $peraccount['ibig_deduct2'] == 1 ) {
        ...
        if($_SESSION['period_type'] == '1H' AND $peraccount['ibig_deduct1'] == 1){
            // if( $peraccount['idacct'] == 82){
            //     $peraccount['ibig_amt'] = 0.00;
            //   }
          
            pagibigContribution($con ,$lastid ,grossContributionpi($con,$lastid),$peraccount['ibig_amt'],$pagibigdivisor);
        }  
        if($_SESSION['period_type'] == '2H' AND $peraccount['ibig_deduct2'] == 1){
            pagibigContribution($con ,$lastid ,grossContributionpi($con,$lastid),$peraccount['ibig_amt'],$pagibigdivisor);
        }
...
function insertpayreg($con ,$cols ,$val, $row, $salary, $semisalary,$paytype){
    ...
    $no_whrs = getmsalary($con,$row) / 2;

    if($row['whrs'] == '0.00'){
        $Qry2           = new Query();
        $Qry2->table    = "tblpayreg";
        $Qry2->selected = "abs_amount = '".$no_whrs ."'";
        $Qry2->fields   = "idacct='".$row['idacct']."' AND id='".$lastid."'";                        
        $Qry2->exe_UPDATE($con);
    }
...
function updatepayreg($con ,$cols ,$val ,$row ,$lastid, $salary,$semisalary,$paytype){
    ...
    $additionals = array("tc_rd","h_sh","h_shrd","h_lh","h_lhrd","h_lsh","h_lshrd","ot_reg","ot_rd","ot_sh","ot_shrd","ot_lh","ot_lhrd","ot_lsh","ot_lshrd","np_npreg","np_rd","np_sh","np_shrd","np_lh","np_lhrd","np_lsh","np_lshrd","npot_npot","npot_rd","npot_sh","npot_shrd","npot_lh","npot_lhrd","npot_lsh","npot_lshrd","adj_late_rd","adj_late_sh","adj_late_shrd","adj_late_lh","adj_late_lhrd","adj_late_lsh","adj_late_lshrd","adj_ut_rd","adj_ut_sh","adj_ut_shrd","adj_ut_lh","adj_ut_lhrd","adj_ut_lsh","adj_ut_lshrd","adj_absent_rd","adj_absent_sh","adj_absent_shrd","adj_absent_lh","adj_absent_lhrd","adj_absent_lsh","adj_absent_lshrd","adj_ot","adj_ot_rd","adj_ot_sh","adj_ot_shrd","adj_ot_lh","adj_ot_lhrd","adj_ot_lsh","adj_ot_lshrd","adj_np","adj_np_rd","adj_np_sh","adj_np_shrd","adj_np_lh","adj_np_lhrd","adj_np_lsh","adj_np_lshrd","adj_npot","adj_npot_rd","adj_npot_sh","adj_npot_shrd","adj_npot_lh","adj_npot_lhrd","adj_npot_lsh","adj_npot_lshrd");
    $novalue =  array("tc_obt","tc_lwop","tc_mlwop");
...
function checkContributionperaccount($con,$row){
    $Qry = new Query();	
    $Qry->table         = "tblaccountjob";
    $Qry->selected      = "idacct,sss_deduct1,sss_deduct2,ibig_deduct1,ibig_deduct2,health_deduct1,health_deduct2,sss_amt,ibig_amt,health_amt";
    $Qry->fields        = "idacct = '".$row['idacct']."'";
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
...
function grossContributionSSS_gross($con,$lastid){
    $Qry = new Query();	
    $Qry->table         = "tblpayreg";
    $Qry->selected      = "(SUM(whrs_amount 
                            + tc_sl_amount 
...
function prevDeductionSSS($con,$lastid){
    $Qry = new Query();	
    $Qry->table         = "tblpayreg";
    $Qry->selected      = "( SUM(whrs_amount) - SUM(late_amount
                            + ut_amount
                            + tc_lwop_amount
                            + tc_mlwop_amount
                            + abs_amount
...
function grossContributionSSS($con,$lastid){
    ...
    if(mysqli_num_rows($rs)>= 1){
        if($row=mysqli_fetch_array($rs)){
            return $row['gross']  + prevDeductionSSS($con,$lastid);
...
function grossContributiontaxable($con,$lastid,$udamount,$sssmatadj){
    ...
    if(mysqli_num_rows($rs)>= 1){
        if($row=mysqli_fetch_array($rs)){
            return ($row['gross'] + getAdditionaltaxable($con, $lastid) - getAdditionaltaxableAddex($con, $lastid) ) + $_SESSION['additionalcont'];
        }
    }
...
function sssContribution($con,$lastid,$amount,$additionalamount){
        ...
        if($amount > 29750.00){ // before  24750
            $Qry->fields        = "id = 53";
        }else{
        ...
        if( $_SESSION['updatedgross'] > $deduction AND ($_SESSION['updatedgross'] - $deduction) > 50){
...
function pagibigContribution($con,$lastid,$amount,$additionalamount,$divisor){
        ...
        if($amount < 1500){
            ...
            //if( $amount > 5000){
                ...
                $er = (5000 * .02) / $divisor;
            // }else{
            //     $ee = ($amount * .02) / $divisor;
            //     $er = ($amount * .02) / $divisor;
            // }
        }
        ...
        if(mysqli_num_rows($rs)>= 1){
            if($row=mysqli_fetch_array($rs)){
                ...
                if($amount > 80000){
                    ...
                    $ee = $amount * $row['prem_rate']/2.0;
                    $er = $amount * $row['prem_rate']/2.0;
                }
...
function allowance($con, $row, $lastid){
    ...
    $id_paydate = $_SESSION['id'];



    $Qry = new Query();	
    $Qry->table         = "tblacctallowance AS ta LEFT JOIN tbltimesheetsummary AS tt ON (ta.`idacct` = tt.`idacct`)";
    $Qry->selected      = "ta.id , tt.idpayperiod,tt.`whrs`,tt.`absref`,amt as amount,firsthalf,secondhalf,idmethod,idallowance as transactionid";
    $Qry->fields        = "ta.idacct = '".$row['idacct']."' AND tt.idpayperiod = '".$id_paydate."'";

    if($period_type == 1){
        $Qry->fields    = "ta.idacct = '".$row['idacct']."' AND firsthalf = 1 AND tt.idpayperiod = '".$id_paydate."'";
    }
    
    if($period_type == 2){
        $Qry->fields    = "ta.idacct = '".$row['idacct']."' AND secondhalf = 1 AND tt.idpayperiod = '".$id_paydate."'";
    }
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
        while($row1=mysqli_fetch_array($rs)){
                ...
                if($row1['idmethod'] == 3 && $row1['firsthalf'] == 1 && $row1['secondhalf'] == 1){
                        $amount = $amount - (($amount/13.04165) * $row1['absref']);
                        if($amount <= 0.00){
                            $amount = '';
                        }
                        else{
                ...
                if($row1['idmethod'] == 3 && $row1['firsthalf'] == 0 && $row1['secondhalf'] == 1){
                    $amount = $amount - (($amount/26.0833) * $row1['absref']);
                    if($amount <= 0.00){
                        $amount = '';
                    }
                    else{
                ...
                if($row1['idmethod'] == 3 && $row1['firsthalf'] == 1 && $row1['secondhalf'] == 0){
                    $amount = $amount - (($amount/26.0833) * $row1['absref']);
                    if($amount <= 0.00){
                        $amount = '';
                    }
...
function getAdditionalSSS($con, $lastid){
    ...
    $additional_classtransids = "161,24,200,217,218,219,181,220,221,222,154,223,71"; // to check NWMH policy
...
function getAdditionaltaxableAddex($con, $lastid){
    $amount = 0;
    $Qry = new Query();	
    $Qry->table         = "tblpayregadditional AS pa LEFT JOIN tblclasstrans AS ct ON pa.transactionid = ct.id LEFT JOIN tblclass AS c ON ct.idclass = c.id";
    $Qry->selected      = "pa.*";
    $Qry->fields        = "pa.payregid = '" . $lastid . "' AND pa.transactionid = 276";
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            $amount =  $amount + $row['amount'];
...
function getreturn($con,$id_paydate,$paygroup){
    ...
    $Qry->fields   = "idpayperiod='".$id_paydate."' AND pr.idpaygrp ='".$paygroup."'  GROUP BY aj.idunit";                        
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            $return[] = $row;
