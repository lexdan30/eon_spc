...
if($preference != ''){
    ...
    $approver1 = $param->acct;
    $approver3 = getApproverss($con, $param->acct, 'approver2', $param->idacct);
    ...
    $approver1 =  $param->acct;
    $approver2 = getApproverss($con, $param->acct, 'approver2', $param->idacct);
    $approver3 = getApproverss($con, $param->acct, 'approver3', $param->idacct);
    $approver4 = '';
}
...
for($i = $begin; $i <= $end; $i->modify('+1 day')){
    ...
    foreach( $param->info->leave_dates as $keys=>$value ){
        $idtimeleavetype= $value->val;
        $ctr2++;
    if( obExists($con, $date, $param->idacct) ){
...
//if($checke){
$return = json_encode( array('status'=>'success','reject'=>$reject, 'err_code' => mysqli_error($con), 'period' =>$id_period, 'date' =>$date) );
// }else{
//     $return = json_encode( array('status'=>'error','reject'=>$reject, 'err_code' => mysqli_error($con)) );
// }
