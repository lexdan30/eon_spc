	...
	if( !empty($param->idleave) ){
		if( !empty($param->from) ){
			if( !empty($param->to) ){
				...
				$Qry->selected  = "
				work_date, tid AS empID, holiday_id, holiday AS holiday_name, idshift,
				csstatus, cs, timein AS `in`, timeout AS `out`, acthrs";
				$Qry->fields    = "tid = '".$param->acct."' AND work_date BETWEEN '".$param->from."' AND '".$param->to."' ORDER BY work_date ASC";
...
function getReghrs($con,$idshift){
	...
	$rs = $Qry->exe_SELECT($con);
	Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getReghrs');
	if(mysqli_num_rows($rs)>= 1){
