...
CASE
WHEN dt.in IS NOT NULL AND dt.out IS NULL THEN 'Incomplete Logs'
WHEN (CAST(dt.in AS TIME)>CAST((SELECT TIME(DATE_ADD(CONCAT(dt.work_date,' ',(SELECT TIME(DATE_ADD(CONCAT(dt.work_date,' ',IF(dt.shiftin IS NULL, sh.stime ,dt.shiftin)),INTERVAL +2 HOUR)))), INTERVAL +29 MINUTE))) AS TIME)) THEN 'Halday'
