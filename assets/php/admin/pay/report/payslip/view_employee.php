...
$Qry1->selected  = "*";
$Qry1->fields    = "id='".$param->id_acct."' AND password='".md5($param->verifypw)."' ";
...
if(mysqli_num_rows($rs1)>= 1){
    ...
    $Qry->table     = "tblpayreg";
    $Qry->selected  = "*";
    $Qry->fields    = "idacct ='".$param->id_acct."' AND idpayperiod = '".$param->id_paydate."'";
    ...
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_assoc($rs)){
            ...
            $payinfo = getPayinfo($con, $param->id_paydate);
           ...
           getpayregAdditionalearnings($con, $row['id']);
           getpayregAdditionaldeductions($con, $row['id']);
...
function  getpayregAdditionalearnings($con, $payregid){
    $Qry = new Query();	
...
function  getpayregAdditionaldeductions($con, $payregid){
    $Qry = new Query();	
    $Qry->table     = "tblpayregadditional";
    $Qry->selected  = "*";
