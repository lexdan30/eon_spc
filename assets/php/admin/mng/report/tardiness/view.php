...
$Qry->selected  = "tid, de.empid, de.empname";
$Qry->fields       = "idunit IN (".$ids.") AND (work_date BETWEEN '".$pay_period['pay_start']."' AND '".$pay_period['pay_end']."') AND (dt.in > shiftin) AND de.empstat != 3";// AND (late > 0)";
...
function getTotalTardy($con,$pay_period,$idacct){
    ...
    $Qry->table="
    (SELECT id FROM vw_dataemployees) AS de LEFT JOIN 
    (SELECT tid, late, work_date, timein AS `in`, stime AS shiftin FROM vw_mng_timesheetfinal) AS dt ON (de.id = dt.tid)";
