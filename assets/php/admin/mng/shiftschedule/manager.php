                        ...
                        if (mysqli_num_rows($rscheck) >= 1)
                        {
                            while ($rowcheck = mysqli_fetch_assoc($rscheck))
                            {
                                ...
                                else
                                {
                                    ...
                                    $Qryb->selected = "idshift='" . $value2->drpndng . "', remarks='" . $remarks . "',type_creator='".$param->typec."', secretary='1'";
...
if ($param->button == 'approve')
{
    ...
    if ($count != 0)
    {
            ...
            if (is_array($value->sched ) || is_object($value->sched ))
            {
                foreach ($value->sched as $key2 => $value2)
                {
                    if($value2->drpndng){
                        ...
                        if(issaved($con,$value,$value2,$param)){
                            $Qryb = new Query();
                            $Qryb->table = "tbldutyroster";
        ...
        //drlock($con,$param->businessunit,$param->payid);
...
function checkIfHasRecall($con, $idacct, $date)
{
    ...
    $rs = $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'checkIfHasRecall');
    return mysqli_num_rows($rs) >= 1;
}
...
function drlock($con,$businessunit,$payid){
    ...
    $Qry->selected="id";
    $Qry->fields="id_department = '".$businessunit."'  AND id_payperiod = '".$payid."'";
    $rs=$Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'drlock');

    if(mysqli_num_rows($rs)>=1){
        $Qry2           = new Query();
        $Qry2->table    = "tbldutyrosterstat";
        $Qry2->selected = "status='1',  date_close ='" . SysDate() . "', time_close='" . SysTime() . "'";
        $Qry2->fields   = "id_department = '".$businessunit."'  AND id_payperiod = '".$payid."'";                    
...
function issaved($con,$value,$value2,$param){
    $Qry2=new Query();
    $Qry2->table="tbldutyroster";
    $Qry2->selected="*";
    $Qry2->fields="idacct='" . $value->id . "' AND date='" . $value2->work_date . "' AND (secretary=1 OR secretary is null) AND (manager=0 OR manager IS NULL)";
    ...
    if(mysqli_num_rows($rs2)>=1){
        ...
        $docnumber = "MAPPR" . $value->id . strtotime($date_create . $time) . $time;
        $Qry = new Query();
        $Qry->table = "tbldutyroster";
        $Qry->selected = "docnumber, 
                                unit, 
