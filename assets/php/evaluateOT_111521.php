<?php
function processOtEvaluation($con){
    $data = array();	
    $Qry = new Query();	
    $Qry->table     = "tbltimeovertime";
    $Qry->selected  = "*,CURRENT_TIMESTAMP() as stamp";
    $Qry->fields    = "id>0 AND (hrs= '0.00' OR hrs IS NULL OR hrs = '') AND DATE_ADD(CURRENT_TIMESTAMP(),INTERVAL -30 DAY) < CONCAT(sdate,' ','00:01') AND evaluated = '0' AND DATE_ADD(CURRENT_TIMESTAMP(),INTERVAL -5 MINUTE) > evaluate_stamp ORDER BY evaluate_stamp asc LIMIT 10";
    ...
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_array($rs)){
           $acthrs = getActualHours($con,$row['idacct'],$row['date']);
                ...
                $Qry3->selected = "hrs='".$acthrs."', evaluate_stamp='".$row['stamp']."'";
                $Qry3->fields   = "id = '".$row['id']."'";                  
                ...
                if($checke){
                    $data[] = array( 
                        ...
                        "evaluatedCount" => evaluateCount($con)
                    );
                }
        }
    }
...
function evaluateCount($con){
    $Qry = new Query();	
    $Qry->table     = "tbltimeovertime";
    $Qry->selected  = "COUNT(*) AS total";
    $Qry->fields    = "id>0 AND (hrs= '0.00' OR hrs IS NULL) AND DATE_ADD(CURRENT_TIMESTAMP(),INTERVAL -1 MONTH) < CONCAT(sdate,' ','00:01')";
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
