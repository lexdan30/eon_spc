				...
				if( $date1 <= $date2 ){
					...
					foreach( $param['info']['leave_dates']  as $keys=>$value ){
						...
						elseif( hasLeave($con, $date, $idacct, $idtimeleavetype) ){
							...
							$Qry->selected 	= "creator, docnumber, idacct, idshift, date, hrs, date_create, approver1, id_payperiod, approver1_date, approver1_time, approver1_stat, stat";
							$id_period		= getTimesheetPayPeriods($con, $date);
							if( (int)$id_period['id'] == 0 ){
								$id_period		= array();
								$id_period		= getLatePayPeriod($con,$date);
							}
							$Qry->fields 	= "'".$param['accountid']."', '".$docnumber."', '".$idacct."', '".$idshift."', '".$date."', '".$hrs."', '".$date_create."','". $approver1 ."', '".$id_period['id']."', '".SysDatePadLeft()."', '".SysTime()."', '1', '1'";
							if( !empty( $remarks ) ){
