...
$Qry->selected  = "pr.id,
                    ...
                    total_ded,
                    net_amount";
$Qry->fields = "pr.idpayperiod = '" . $id_paydate . "'  AND pr.type = '" . $type  . "' " .$search . "  GROUP BY pr.id  ORDER BY empname LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize ."";
...
function grandTotal($con,$idpayperiod,$param){
    ...
    $Qry->fields        = "pr.idpayperiod = '" . $id_paydate . "'  AND pr.type = '" . $type  . "'" .$search . "";
    $rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
...
function getTotal($con,$idpayperiod,$param){
    ...
    $Qry->selected  = "pr.id,
                        ...
                        total_ded,
                        net_amount";
    $Qry->fields     = "pr.idpayperiod = '" . $id_paydate . "' " .$search . "GROUP BY pr.id ";
    $rs = $Qry->exe_SELECT($con);
		if(mysqli_num_rows($rs)>= 1){
