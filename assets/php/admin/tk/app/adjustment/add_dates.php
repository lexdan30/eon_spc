...
if( !empty($param->acct) ){
		if( !empty($param->from) ){
			if( !empty($param->to) ){
				...
				$Qry = new Query();	
				$Qry->table     = "(SELECT work_date, id, idshift, csstatus, cs, holidaytype, alias, FPidshift, timeout, timein FROM vw_timesheet) as a
				LEFT JOIN (SELECT idacct, munid, provcode FROM tblaccountjob) AS w ON (w.idacct = a.id)
				LEFT JOIN (SELECT `date`, munid, provcode FROM tblholidays) AS g ON (g.date = a.work_date)";
				$Qry->selected  = "
				a.work_date, 
				a.id,
				a.idshift,
					...
					while($row=mysqli_fetch_assoc($rs)){
						...
						if( !empty($row['holidaytype']) ){
							if(!empty($row['holidayProvid'])){
								...
								{
									$shift_info[1] = '<p style="color:red; font-weight: 900;"> '.ucwords(strtolower($row['alias'])).'H ' .$shift_info[1]. '</p>';
									$shift_info[1] = strip_tags($shift_info[1]);
									$hol = true;
...
function getsched($con, $id){
    ...
    $Qry = new Query();	
    $Qry->table     = "tblshift";
    $Qry->selected  = "stime, ftime, breakin, breakout, sstime, sftime";
    $Qry->fields    = "id='".$id."'";
    $rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
...
function getActLogss($con, $id, $date,$idshift){
    ...
    $Qry->selected  = "id, idshift, timein, timeout, timein2, timeout2, `date`, date_out";
