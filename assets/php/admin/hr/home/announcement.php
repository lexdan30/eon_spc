...
$Qry = new Query();
$Qry->table = "(SELECT id, idcreator, event_title, event_desc, efrom, eto, canview, atype, filename, isactive FROM `tblcompanyact`) as tblc LEFT JOIN (SELECT id, fname, lname FROM tblaccount) as tbla ON (tblc.idcreator = tbla.id)";
...
while($row=mysqli_fetch_assoc($rs)){
	...
	if( !empty( $row['filename'] ) ){
		...
		if($filesize = formatSizeUnits($file))
		{
			$f=true;
		}
