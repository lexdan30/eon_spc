...
if( !empty( $param->accountid ) && !empty($str) ){
	...
	$Qry2->fields    = "tid IN (".$str.") AND ut <> '0.00' AND ut <> '0' AND work_date BETWEEN '".$first_day_this_month."' AND '".$last_day_this_month."' GROUP BY empid ORDER BY COUNT(empid) DESC LIMIT 5 ";
	$rs2 			= $Qry2->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs));
	if( mysqli_num_rows($rs2) >= 1 ){
