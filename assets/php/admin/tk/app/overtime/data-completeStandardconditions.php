...
function getActualHours( $con,$idacct,$date,$edate){
	$sdate			= substr($date,0,10); 
	$fdate			=  substr($edate,0,10); 
	$stime			= substr(getschedinfo($con,$idacct,$date)['out'],0,5);
	$ftime			= substr(getschedinfo($con,$idacct,$date)['actout'],0,5);	

	if(getschedinfo($con,$idacct, $param->info->otsdate)['fallonweekdayholiday'] && getschedinfo($con,$idacct, $param->info->otsdate)['site'] != '1' && (strtotime( $sdate.' '.$stime.'' ) < strtotime( $sdate.' '.'07:00'.'' )) &&  getschedinfo($con,$idacct, $param->info->otsdate)['driver'] == false){
		$stime = '07:00';
	} 

	if(getschedinfo($con,$idacct, $param->info->otsdate)['flexi'] && getschedinfo($con,$idacct, $param->info->otsdate)['site'] != '1' && getschedinfo($con,$idacct, $param->info->otsdate)['joblevel'] == 'Mgr' && getschedinfo($con,$idacct, $param->info->otsdate)['fallonweekdayholiday'] == false){ 
	...
	}elseif(getschedinfo($con,$idacct, $param->info->otsdate)['fallonweekdayholiday'] && getschedinfo($con,$idacct, $param->info->otsdate)['joblevel'] == 'RnF'){
...
function getschedinfo($con, $id, $date){ 
	...
	$Qry->selected  = "stime,ftime,idshift,breakin,breakout,site,idlvl,timeout";
...
END ) AS breakoutref,  `e`.`site` AS `site`,  `e`.`idlvl` AS `idlvl`, `a`.timeout
	";
