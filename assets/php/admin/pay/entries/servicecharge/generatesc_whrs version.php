...
$Qry->selected  = "a.id,a.empname,IF((whrs - SUM(late+ut+`abs`+tc_lwop))<0,0,(whrs - SUM(late+ut+`abs`+tc_lwop)))  AS totwhrs,(adj_absent - SUM(adj_late + adj_ut + adj_lwop)) + (".$adj_leaves.") AS adj_totwhrs"; //IF(pa.transactiontype = 'UD',pa.amount,0.00) AS uniondues //+ (".$leaves.")
$Qry->fields    = "(p.idpayperiod IN (".$ids.")) AND a.sc_stat='1' GROUP by a.id ORDER BY a.empname LIMIT " .$param->pagination->pageSize. " OFFSET " . ($param->pagination->currentPage - 1) * $param->pagination->pageSize; //"; //a.sc_stat='1' AND 
...
$myData = array('status' => 'success', 'result' => $data,'query' =>  $Qry->fields,
 'totalItems' => getTotal($con , $where, $leaves),'totalwhrs' => $totwrhs,'totbpays' => $totwrhs_adj, 'prev_scrate' => $prev_scrate,
 'totbp_amount' => $totbp_amount, 'netsc' => $netsc, 'totushare' => $totushare,'total' => $total,'scrate' => $scrate,
 ...
 'scrate' => $scrate,
 'gettotalemp_sc' => getTotalEmpDetails($con,$searchall,$scrate,$leaves),
...
function getTotal($con,$search,$leaves){
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="a.empname,(whrs - SUM(late+ut+`abs`+tc_lwop)) AS totwhrs"; // + (".$leaves.")
	$Qry->fields = "a.sc_stat='1'  AND (whrs - SUM(late+ut+`abs`+tc_lwop)) > 0 ". $search;
	$rs = $Qry->exe_SELECT($con);
	return mysqli_num_rows($rs);
}

function getTotalEmpDetails($con,$search,$scrate,$leaves){
    $gettotalemp_sc = 0;
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="a.empname,IF((whrs - SUM(late+ut+`abs`+tc_lwop))<0,0,(whrs - SUM(late+ut+`abs`+tc_lwop))) AS totwhrs"; //+ (".$leaves.")
	$Qry->fields =  "a.sc_stat='1' ". $search;
	$rs = $Qry->exe_SELECT($con);
    if(mysqli_num_rows($rs)>= 1){
        while($row=mysqli_fetch_array($rs)){
...
function gettotalwhrs($con,$search){
	...
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="(SUM(whrs) - (SUM(late)+SUM(ut)+SUM(`abs`)+SUM(tc_lwop))) AS totwhrs"; // + (".$leaves.")
	$Qry->fields =  "a.sc_stat='1' AND (whrs - ((late)+(ut)+(`abs`)+(tc_lwop))) > 0 ". $search;
	$rs = $Qry->exe_SELECT($con);
	if(mysqli_num_rows($rs)>= 1){
...
function gettotalwhrs_adj($con,$search){
	...
	$Qry = new Query();	
	$Qry->table ="vw_dataemployees AS a LEFT JOIN tbltimesheetsummary AS p ON a.id = p.idacct";
	$Qry->selected ="(SUM(adj_absent) - (SUM(adj_late) + SUM(adj_ut) + SUM(adj_lwop))) + (".$adj_leaves.") AS adj_totwhrs";
	$Qry->fields =  "a.sc_stat='1' ". $search;
	$rs = $Qry->exe_SELECT($con);
	if(mysqli_num_rows($rs)>= 1){
        if($row=mysqli_fetch_array($rs)){
