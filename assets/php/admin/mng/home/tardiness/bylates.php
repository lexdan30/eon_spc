...
$pie_data	= getLates($con,$ids,$param->accountid);
...
function getEmp($con, $ids,$idacct){
	...
	if( mysqli_num_rows($rs) >= 1 ){
		while($row=mysqli_fetch_assoc($rs)){
            ...
            $Qry2->fields    = " MONTH(work_date) = MONTH(CURRENT_DATE()) AND YEAR(work_date) = YEAR(CURRENT_DATE()) and late <> '0.00' AND late <> '0' AND tid IN (".$row['id'].") group by empname";
...
// 	return $data;
// }
function getLates($con, $ids,$idacct){
	$data=array();
	$Qry 			= new Query();	
	$Qry->table     = "vw_dataemployees";
	$Qry->selected  = "id";
    $Qry->fields    = "(idunit IN (".$ids.") OR idsuperior='".$idacct."') AND id != '".$idacct."'";
	$rs 			= $Qry->exe_SELECT($con);
    Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getLates');
	if( mysqli_num_rows($rs) >= 1 ){
		while($row=mysqli_fetch_assoc($rs)){
            ...
            $Qry2->fields    = " MONTH(work_date) = MONTH(CURRENT_DATE()) AND YEAR(work_date) = YEAR(CURRENT_DATE()) and late <> '0.00' AND late <> '0' AND tid IN (".$row['id'].")";
            $rs2 			= $Qry2->exe_SELECT($con);
            Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs), 'getLates(Qry2)');
            if( mysqli_num_rows($rs2) >= 1 ){
                while($row2=mysqli_fetch_assoc($rs2)){
                    array_push($data,$row2['ctr']);
                }
