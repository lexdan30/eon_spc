	...
	if(!empty($param->info->name)){	
		if(!empty($param->info->alias)){
			if(!empty($param->info->rate)){
					...
					if( checkOvertimeName($con,$param->info->name, $param->info->id) ){
					...
					if( checkOvertimeAlias($con,$param->info->alias, $param->info->id) ){
					...
					$Qry3->selected = "name='".$param->info->name."',alias='".$param->info->alias."',rate='".$param->info->rate."'";
