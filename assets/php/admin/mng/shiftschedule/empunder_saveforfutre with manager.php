...
require_once('../../../classPhp.php');
    ...
    if($param->under != ''){
        ...
        $Qry->fields="a.idemptype = 1 AND ((b.idsuperior='".$idhead."') OR a.id = '".$idhead."') ORDER BY u.name,e.joblvl,a.lname";
        ...
        $Qry->fields="a.idemptype = 1 AND ((b.idsuperior='".$param->accountid."') OR a.id = '".$param->accountid."') ORDER BY a.lname";
        $rs=$Qry->exe_SELECT($con);
        //echo $Qry->fields;
        Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs));
        if(mysqli_num_rows($rs)>=1){
