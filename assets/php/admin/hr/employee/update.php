																		...
																		if($checke){
																			...
																			if( array_key_exists('file',$_FILES) ){		
																				if( array_key_exists(0,$_FILES['file']['name']) ){
																					...
																					convertImgToWebp($param['info']['empid'], $folder_path.$save_name, $extMove);
																				}
																			}
...
function convertImgToWebp($filename, $pathname, $imgtype) {
	switch($imgtype) {
		case ".jpg":
		case ".JPG":
		case ".jpeg":
			$img = imagecreatefromjpeg($pathname);
			break;
	default:
			$img = imagecreatefrompng($pathname);
			break;
	}
    $w = imagesx($img);
    $h = imagesy($img);
    ...
    imagewebp($webp, "pix\\".$filename.".webp", 80);
    imagedestroy($img);
    imagedestroy($webp);
}
...
function updateInfo($con, $param){
	...
	if( ("".$param['info']['isSC']['checked']) == "true" ){
		$param['info']['isSC']['id'] = 1;
	}
	...
	$Qry->selected 	= " idacct='".$param['info']['idacct']."',
						...
						sc_stat='".(int)$param['info']['isSC']['id']."',
