...
$Qry->selected  = "
planned_start, planned_end, `date`, act_hrs, idacct,
planned_date_end, idacct, docnumber, empid, empname,
remarks, approve_hr, ot_status, date_approve, id, planned_hrs";
...
function getActualHours($con,$idacct,$date,$fdate,$schedInfo){
	...
	if((!empty($stime) && !empty($ftime))){
		...
		if($schedInfo['flexi'] && $schedInfo['site'] != '1' && $schedInfo['joblevel'] == 'Mgr' &&$schedInfo['fallonweekdayholiday'] == false){ 
			...
			Log::v(strlen($con->error) > 0 ? " Err: ".$con->error : ""." num_count: ".mysqli_num_rows($rs2), 'getActualHours(else)');
